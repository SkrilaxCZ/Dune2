...
static bool Cutscene_InputSkipScene()
{
	Input_Tick(true);
...
static void Cutscene_CopyScreen()
{
	Video_DrawWSA(NULL, 0, 0, 0, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
	Video_Tick();
}

static uint8 Cutscene_GetPixel(uint16 x, uint16 y)
{
	if (y >= SCREEN_HEIGHT)
		return 0;
	if (x >= SCREEN_WIDTH)
...
static void Cutscene_PutPixel(uint16 x, uint16 y, uint8 colour)
{
	if (y >= SCREEN_HEIGHT)
		return;
	if (x >= SCREEN_WIDTH)
...
static void Cutscene_DrawText(char* str, int left, int top, uint8 fg, uint8 bg)
{
...
static void Cutscene_DrawText_Wrapper(const char* str, int x, int y)
{
	const char* s = str;
...
static void Cutscene_Screen_FadeIn(uint16 xSrc, uint16 ySrc, uint16 xDst, uint16 yDst, uint16 width, uint16 height, Screen screenSrc, Screen screenDst)
{
	uint16 offsetsY[100];
	uint16 offsetsX[40];
	int x, y;
...
static void Cutscene_Screen_FadeIn2(int16 x, int16 y, int16 width, int16 height, Screen screenSrc, Screen screenDst, uint16 delay, bool skipNull)
{
	Screen oldScreenID;
	uint16 i;
	uint16 j;
...
static void Cutscene_SetPaletteAnimated(uint8* palette, int16 ticksOfAnimation)
{
	bool progress;
	int16 diffPerTick;
	int16 tickSlice;
	...
	if (g_paletteActive == NULL || palette == NULL)
		return;
...
static void GameLoop_PlaySoundEffect(uint8 animation)
{
	...
	if (soundEffect->animationID > animation || soundEffect->wait > s_var_8068)
		return;

	Audio_PlaySoundCutscene((SoundID)soundEffect->voiceID);
...
static void GameLoop_PlaySubtitle(uint8 animation)
{
	...
	/* GUI_DrawFilledRectangle(0, subtitle->top == 85 ? 0 : subtitle->top, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, 0); */
	{
		...
		unsigned char* screen = (unsigned char*)GFX_Screen_Get_ByIndex(SCREEN_0);

		memset(screen + SCREEN_WIDTH * top, 0, SCREEN_WIDTH * (SCREEN_HEIGHT - top));
	}

	if (s_var_8062 != 0xFFFF && s_houseAnimation_currentSubtitle != 0)
	{
		uint16 loc06 = s_var_8062 + s_houseAnimation_currentSubtitle;
		Audio_PlayVoice((VoiceID)loc06);

		if (g_feedback[loc06].messageId != 0)
		{
			if ((g_feedback[loc06].messageId == 1) || (g_feedback[loc06].messageId == 2 && g_enable_subtitles))
	...
	if (g_playerHouseID != HOUSE_INVALID || s_houseAnimation_currentSubtitle != 2)
		return;
 ...
 */
static bool GameLoop_PalettePart_Update(bool finishNow)
{
	Audio_Poll();

	if (s_palettePartDirection == PPD_STOPPED)
		return false;

	if (s_paletteAnimationTimeout >= Timer_GetTicks() && !finishNow)
		return false;
...
static void GameCredits_Play(char* data, uint16 windowID, Screen memory, Screen screenID, uint16 delay)
{
	...
	while (true)
	{
		...
		while ((g_curWidgetHeight / 6) + 2 > stringCount && *data != 0)
		{
			...
			*data = '\0';
			if (strings[stringCount].separator != 0)
				data++;
		...
		for (loc02 = 0; loc02 < stringCount; loc02++)
		{
			if ((int16)strings[loc02].y < g_curWidgetHeight)
			{
				...
				if (strings[loc02].charHeight != g_fontCurrent->height)
					Font_Select(g_fontNew6p);
		...
		{
			if (strings[stringCount - 1].y + strings[stringCount - 1].charHeight < g_curWidgetYBase + g_curWidgetHeight)
				loc10 = true;
		}
...
void GameLoop_GameCredits(HouseType houseID)
{
	...
	memory = (uint8*)GFX_Screen_Get_ByIndex(SCREEN_2);
	...
	for (;; sleepIdle())
	{
		...
		GameCredits_Play((char*)s_buffer_1832, 20, SCREEN_1, SCREEN_2, 6);
...
{
	const HouseType h = g_playerHouseID;
