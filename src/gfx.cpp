...
AspectRatioCorrection g_aspect_correction = ASPECT_RATIO_CORRECTION_AUTO;
...
void GFX_Init()
{
	...
	for (i = 0; i < 5; i++)
		totalSize += GFX_Screen_GetSize_ByIndex((Screen)(i * 2));
	...
	screenBuffers = (uint8*)calloc(1, totalSize);

	for (i = 0; i < 5; i++)
	{
		s_screenBuffer[i] = screenBuffers;
		screenBuffers += GFX_Screen_GetSize_ByIndex((Screen)(i * 2));
	}
...
void GFX_DrawSprite(uint16 spriteID, uint16 x, uint16 y, uint8 houseID)
{
	...
	wptr = (uint8*)GFX_Screen_GetActive();
...
void GFX_Screen_Copy2(int16 xSrc, int16 ySrc, int16 xDst, int16 yDst, int16 width, int16 height, Screen screenSrc, Screen screenDst, bool skipNull)
{
	...
	if (SCREEN_WIDTH - xSrc - width < 0)
		width = SCREEN_WIDTH - xSrc;
	if (SCREEN_HEIGHT - ySrc - height < 0)
		height = SCREEN_HEIGHT - ySrc;
	if (SCREEN_WIDTH - xDst - width < 0)
		width = SCREEN_WIDTH - xDst;
	if (SCREEN_HEIGHT - yDst - height < 0)
		height = SCREEN_HEIGHT - yDst;

	if (xSrc < 0 || xSrc >= SCREEN_WIDTH)
		return;
	if (xDst < 0 || xDst >= SCREEN_WIDTH)
		return;
	if (ySrc < 0 || ySrc >= SCREEN_HEIGHT)
		return;
	if (yDst < 0 || yDst >= SCREEN_HEIGHT)
		return;
	...
	src = (uint8*)GFX_Screen_Get_ByIndex(screenSrc);
	dst = (uint8*)GFX_Screen_Get_ByIndex(screenDst);
