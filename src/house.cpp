...
void GameLoop_House()
{
	...
	if (tickMissileCountdown && g_houseMissileCountdown != 0)
	{
		...
		const VoiceID voiceID = (const VoiceID)(VOICE_MISSILE_LAUNCHED + (int)g_houseMissileCountdown - 1);
	...
	if (tickStarportAvailability)
	{
		 ...
		 */
		const UnitType type = (const UnitType)Tools_RandomLCG_Range(0, UNIT_MAX - 1);
	...
	if (tickReinforcement)
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			if (g_scenario.reinforcement[i].unitID == UNIT_INDEX_INVALID)
				continue;
	...
	while (true)
	{
		...
		if (tickHouse)
			House_EnsureHarvesterAvailable((uint8)h->index);
		...
		if (tickHouse)
		{
			...
			if (h->harvestersIncoming > 0 && Unit_CreateWrapper((uint8)h->index, UNIT_HARVESTER, 0) != NULL)
...
uint8 House_StringToType(const char* name)
{
	...
	{
		if (stricmp(g_table_houseInfo[index].name, name) == 0)
			return index;
	}
...
void House_EnsureHarvesterAvailable(uint8 houseID)
{
	...
	if (Unit_IsTypeOnMap(houseID, UNIT_HARVESTER))
		return;
	...
	if (Unit_CreateWrapper(houseID, UNIT_HARVESTER, Tools_Index_Encode(s->o.index, IT_STRUCTURE)) == NULL)
		return;
...
void House_CalculatePowerAndCredit(House* h)
{
	...
	while (true)
	{
		...
		/* Only count structures that are placed on the map, not ones we are building. */
