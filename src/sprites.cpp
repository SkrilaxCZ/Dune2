...
static const uint8* Sprites_GetSprite(const uint8* buffer, uint16 index)
{
	...
	if (buffer == NULL)
		return NULL;
	if (READ_LE_UINT16(buffer) <= index)
		return NULL;
 ...
 */
static void Sprites_Load(SearchDirectory dir, const char* filename, int start, int end)
{
	uint8* buffer;
	uint16 count;
	uint16 i;

	buffer = (uint8*)File_ReadWholeFile_Ex(dir, filename);
...
void Sprites_LoadTiles()
{
	...
	Script_LoadFromFile("UNIT.EMC", g_scriptUnit, g_scriptFunctionsUnit, (uint8*)GFX_Screen_Get_ByIndex(SCREEN_2));
}
 ...
 */
static uint32
Sprites_LoadCPSFile(SearchDirectory dir, const char* filename,
                    Screen screenID, uint8* palette)
{
	...
	buffer = (uint8*)GFX_Screen_Get_ByIndex(screenID);
	...
	buffer2 = (uint8*)GFX_Screen_Get_ByIndex(screenID);
 ...
 */
uint16
Sprites_LoadImage(SearchDirectory dir, const char* filename,
                  Screen screenID, uint8* palette)
{
	uint8 index;
	uint32 header;
...
static void InitRegions()
{
	...
	Ini_GetString("INFO", "TOTAL REGIONS", NULL, textBuffer, lengthof(textBuffer) - 1, (char*)g_fileRegionINI);
...
{
	if (spriteID > g_veiledSpriteID)
		return true;
	if (spriteID < g_veiledSpriteID - 15)
		return true;
...
{
	static MentatID l_mentatID = MENTAT_MAX;
	static HouseType l_houseID = HOUSE_INVALID;
