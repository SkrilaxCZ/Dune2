...
static void Unit_Rotate(Unit* unit, uint16 level)
{
	...
	if (unit->orientation[level].speed == 0)
		return;
	...
	if (Orientation_256To16(newCurrent) == Orientation_256To16(current) && Orientation_256To8(newCurrent) == Orientation_256To8(current))
		return;
...
{
	if ((u->speedPerTick == 255) || (u->speedRemainder + Tools_AdjustToGameSpeed(u->speedPerTick, 1, 255, false) > 0xFF))
	{
...
void GameLoop_Unit()
{
	...
	while (true)
	{
		...
		if (u->nextActionID == ACTION_INVALID)
			continue;
...
uint8 Unit_StringToType(const char* name)
{
	...
	{
		if (stricmp(g_table_unitInfo[type].o.name, name) == 0)
			return type;
	}
...
uint8 Unit_ActionStringToType(const char* name)
{
	...
	{
		if (stricmp(g_table_actionInfo[type].name, name) == 0)
			return type;
	}
...
uint8 Unit_MovementStringToType(const char* name)
{
	...
	{
		if (stricmp(g_table_movementTypeName[type], name) == 0)
			return type;
	}
...
Unit* Unit_Create(uint16 index, uint8 typeID, uint8 houseID, tile32 position, int8 orientation)
{
	...
	Unit_SetAction(u, (ActionType)((houseID == g_playerHouseID) ? ui->o.actionsPlayer[3] : ui->actionAI));
...
bool Unit_IsTypeOnMap(uint8 houseID, uint8 typeID)
{
	...
	for (i = 0; i < g_unitFindCount; i++)
	{
		...
		u = g_unitFindArray[i];
		if (houseID != HOUSE_INVALID && Unit_GetHouseID(u) != houseID)
			continue;
		if (typeID != UNIT_INVALID && u->o.type != typeID)
			continue;
		if (g_validateStrictIfZero == 0 && u->o.flags.s.isNotOnMap)
			continue;
...
void Unit_SetAction(Unit* u, ActionType action)
{
	...
	if (u == NULL)
		return;
	if (u->actionID == ACTION_DESTRUCT || u->actionID == ACTION_DIE || action == ACTION_INVALID)
		return;
...
void Unit_Sort()
{
	...
	for (i = 0; i < g_unitFindCount - 1; i++)
	{
		...
		y1 = u1->o.position.y;
		y2 = u2->o.position.y;
		if (g_table_unitInfo[u1->o.type].movementType == MOVEMENT_FOOT)
			y1 -= 0x100;
		if (g_table_unitInfo[u2->o.type].movementType == MOVEMENT_FOOT)
			y2 -= 0x100;
...
uint16 Unit_IsValidMovementIntoStructure(Unit* unit, Structure* s)
{
	...
	if (Unit_GetHouseID(unit) != s->o.houseID)
	{
		...
		if (unit->o.type == UNIT_SABOTEUR && unit->targetMove == structEnc)
			return 2;
		...
		if (ui->movementType == MOVEMENT_FOOT && si->o.flags.conquerable)
		{
			...
			if (unit->actionID == ACTION_ATTACK)
				return 1;
	...
	if ((si->enterFilter & (1 << unit->o.type)) == 0)
		return 0;

	/* TODO -- Not sure. */
	if (s->o.script.variables[4] == unitEnc)
		return 2;
...
void Unit_SetDestination(Unit* u, uint16 destination)
{
	...
	if (AI_IsBrutalAI((HouseType)u->o.houseID))
	{
	...
	if (!Tools_Index_IsValid(destination))
		return;
	if (u->targetMove == destination)
		return;
	...
	if (Tools_Index_GetType(destination) == IT_TILE)
	{
		...
		{
			if (u != u2)
				destination = Tools_Index_Encode(u2->o.index, IT_UNIT);
		}
		else
		{
			s = Structure_Get_ByPackedTile(packed);
			if (s != NULL)
				destination = Tools_Index_Encode(s->o.index, IT_STRUCTURE);
		}
	}
...
uint16 Unit_GetTargetUnitPriority(Unit* unit, Unit* target)
{
	...
	if (!target->o.flags.s.allocated)
		return 0;
	if ((target->o.seenByHouses & (1 << Unit_GetHouseID(unit))) == 0)
		return 0;

	if (House_AreAllied(Unit_GetHouseID(unit), Unit_GetHouseID(target)))
		return 0;

	unitInfo = &g_table_unitInfo[unit->o.type];
	targetInfo = &g_table_unitInfo[target->o.type];

	if (!targetInfo->o.flags.priority)
		return 0;

	if (targetInfo->movementType == MOVEMENT_WINGER)
	{
		if (!unitInfo->o.flags.targetAir)
			return 0;
		if (target->o.houseID == g_playerHouseID && !Map_IsPositionUnveiled(Tile_PackTile(target->o.position)))
			return 0;
	}

	if (!Map_IsValidPosition(Tile_PackTile(target->o.position)))
		return 0;
	...
	{
		if (targetInfo->fireDistance >= distance)
			return 0;
	}
	...
	if (AI_IsBrutalAI((HouseType)Unit_GetHouseID(unit)))
	{
...
uint16 Unit_FindClosestRefinery(Unit* unit)
{
	...
	while (true)
	{
		...
		if (s2 == NULL)
			break;
		if (s2->state != STRUCTURE_STATE_BUSY)
			continue;
...
Unit* Unit_FindBestTargetUnit(Unit* u, uint16 mode)
{
	...
	while (true)
	{
		...
		if (mode != 0 && mode != 4)
		{
			if (mode == 1)
			{
				if (Tile_GetDistance(u->o.position, target->o.position) > distance)
					continue;
			}
			if (mode == 2)
			{
				if (Tile_GetDistance(position, target->o.position) > distance)
					continue;
			}
		}
...
static uint16 Unit_Sandworm_GetTargetPriority(Unit* unit, Unit* target)
{
	...
	if (unit == NULL || target == NULL)
		return 0;
	if (!Map_IsPositionUnveiled(Tile_PackTile(target->o.position)))
		return 0;
	if (!g_table_landscapeInfo[Map_GetLandscapeType(Tile_PackTile(target->o.position))].isSand)
		return 0;

	switch (g_table_unitInfo[target->o.type].movementType)
	{
	case MOVEMENT_FOOT: res = 0x64;
		break;
	case MOVEMENT_TRACKED: res = 0x3E8;
		break;
	case MOVEMENT_HARVESTER: res = 0x3E8;
		break;
	case MOVEMENT_WHEELED: res = 0x1388;
		break;
	default: res = 0;
		break;
	}

	if (target->speed != 0 || target->fireDelay != 0)
		res *= 4;
...
{
	if (unit == NULL || !Tools_Index_IsValid(encoded))
		return;
	if (unit->targetAttack == encoded)
		return;
...
bool Unit_Deviate(Unit* unit, uint16 probability, uint8 houseID)
{
	...
	if (probability == 0)
		probability = g_table_houseInfo[unit->o.houseID].toughness;
	...
	if (Tools_Random_256() >= probability)
		return false;
	...
	if (g_playerHouseID == houseID)
	{
		...
		if (AI_IsBrutalAI((HouseType)unit->o.houseID))
	...
	{
		if (AI_IsBrutalAI((HouseType)houseID))
		{
...
bool Unit_Move(Unit* unit, uint16 distance)
{
	...
	if (unit == NULL || !unit->o.flags.s.used)
		return false;
	...
	if ((newPosition.x == unit->o.position.x) && (newPosition.y == unit->o.position.y))
		return false;
	...
	if (unit->o.type == UNIT_SONIC_BLAST)
	{
		...
		else
		{
			...
			if (s != NULL)
			{
				if (s->o.houseID != g_playerHouseID && !House_AreAllied(unit->o.houseID, s->o.houseID))
		...
		if (ret)
		{
			...
			else if (ui->flags.isGroundUnit)
			{
				if (currentDestination.x != 0 || currentDestination.y != 0)
					newPosition = currentDestination;
	...
	if (isSpiceBloom)
		Map_Bloom_ExplodeSpice(packed, Unit_GetHouseID(unit));
...
bool Unit_Damage(Unit* unit, uint16 damage, uint16 range)
{
	...
	if (unit == NULL || !unit->o.flags.s.allocated)
		return false;
	...
	if (unit->o.hitpoints >= ui->o.hitpoints / 2)
		return false;
...
void Unit_UntargetEncodedIndex(uint16 encoded)
{
	...
	while (true)
	{
		...
		if (u->targetMove == encoded)
			u->targetMove = 0;
		if (u->targetAttack == encoded)
			u->targetAttack = 0;
		if (u->o.script.variables[4] == encoded)
...
void Unit_UntargetMe(Unit* unit)
{
	...
	while (true)
	{
		...
		if (s->o.type != STRUCTURE_TURRET && s->o.type != STRUCTURE_ROCKET_TURRET)
			continue;
		if (s->o.script.variables[2] == encoded)
			s->o.script.variables[2] = 0;
	}
...
void Unit_SetOrientation(Unit* unit, int8 orientation, bool rotateInstantly, uint16 level)
{
	...
	if (unit->orientation[level].current == orientation)
		return;

	/* Carryalls sometimes circle around a tile when
	 * delivering reinforcements due to the turning radius.  Make them
	 * go straight ahead a little further before turning around.
	 */
	if ((unit->o.type == UNIT_CARRYALL) && (level == 0))
	{
...
Unit* Unit_CreateWrapper(uint8 houseID, UnitType typeID, uint16 destination)
{
	...
	if (carryall == NULL)
	{
		if (typeID == UNIT_HARVESTER && h->harvestersIncoming == 0)
...
uint16 Unit_FindTargetAround(uint16 packed)
{
	...
	if (Map_GetLandscapeType(packed) == LST_BLOOM_FIELD)
		return packed;
...
bool Unit_IsTileOccupied(Unit* unit)
{
	...
	if (unit->o.type == UNIT_SANDWORM || ui->movementType == MOVEMENT_WINGER)
		return false;

	unit2 = Unit_Get_ByPackedTile(packed);
	if (unit2 != NULL && unit2 != unit)
	{
		if (House_AreAllied(Unit_GetHouseID(unit2), Unit_GetHouseID(unit)))
			return true;
		if (ui->movementType != MOVEMENT_TRACKED)
			return true;
		if (g_table_unitInfo[unit2->o.type].movementType != MOVEMENT_FOOT)
			return true;
	}
	...
	case UNIT_MISSILE_TROOPER:
		{
			...
			Audio_PlaySoundAtTile((SoundID)ui->bulletSound, position);
	...
	case UNIT_SONIC_BLAST:
		{
			...
			if (damage > 15)
				bullet->o.flags.s.bulletIsBig = true;

			if ((bullet->o.seenByHouses & (1 << g_playerHouseID)) != 0)
				return bullet;
...
void Unit_EnterStructure(Unit* unit, Structure* s)
{
	...
	if (Unit_IsSelected(unit))
	{
		...
		/*  When a Unit enters a Structure, the last tile the Unit was on becomes selected rather than the entire Structure. */
		if (!Unit_AnySelected())
	...
	if (House_AreAllied(s->o.houseID, Unit_GetHouseID(unit)))
	{
		...
		{
			uint16 countDown = ((ui->o.hitpoints - unit->o.hitpoints) * 256 / ui->o.hitpoints) * (ui->o.buildTime << 6) / 256;
			...
			/*  Units can be ejected prematurely, so don't restore hitpoints just yet. */
			/* unit->o.hitpoints = ui->o.hitpoints; */
			unit->o.flags.s.isSmoking = false;
			unit->spriteOffset = 0;
		}
	...
	if (s->o.hitpoints < si->o.hitpoints / 4)
	{
		...
		if (g_playerHouseID == s->o.houseID)
			Audio_PlayVoice(VOICE_HARKONNEN_STRUCTURE_CAPTURED + (int)g_playerHouseID);
		else
		...
		/* recalculate the power and credits for the house losing the structure. */
		...
		/*  When taking over a structure, untarget it. Else you will destroy the structure you just have taken over very easily */
		Structure_UntargetMe(s);
	}
	else
	{
		uint16 damage = min(unit->o.hitpoints * 2, s->o.hitpoints / 2);
...
static bool Unit_StructureInRange(const Unit* unit, const Structure* s, uint16 distance)
{
	const StructureLayout layout = (const StructureLayout)g_table_structureInfo[s->o.type].layout;
	tile32 curPosition;

	/* Units on guard normally only check the distance to
	 *
...
static Structure* Unit_FindBestTargetStructure(Unit* unit, uint16 mode)
{
	...
	while (true)
	{
		...
		if (mode != 0 && mode != 4)
		{
			if (mode == 1)
			{
				if (!Unit_StructureInRange(unit, s, distance))
					continue;
			}
			else
			{
				...
				if (Tile_GetDistance(position, curPosition) > distance * 2)
					continue;
			}
		}
...
int16 Unit_GetTileEnterScore(Unit* unit, uint16 packed, uint16 direction)
{
	...
	if (!Map_IsValidPosition(packed) && ui->movementType != MOVEMENT_WINGER)
		return 256;
	...
	if (u != NULL && u != unit && unit->o.type != UNIT_SANDWORM)
	{
		...
		if (House_AreAllied(Unit_GetHouseID(u), Unit_GetHouseID(unit)))
			return 256;
	...
	if (unit->o.type == UNIT_SABOTEUR && type == LST_WALL)
	{
		if (!House_AreAllied(g_map[packed].houseID, Unit_GetHouseID(unit)))
			res = 255;
	}
...
void Unit_UpdateMap(uint16 type, Unit* unit)
{
	...
	if (unit == NULL || unit->o.flags.s.isNotOnMap || !unit->o.flags.s.used)
		return;
	...
	if (unit->o.flags.s.bulletIsBig || unit->o.flags.s.isSmoking || (unit->o.type == UNIT_HARVESTER && unit->actionID == ACTION_HARVEST))
		radius = 33;
...
uint16 Unit_GetTargetStructurePriority(Unit* unit, Structure* target)
{
	...
	if (House_AreAllied(Unit_GetHouseID(unit), target->o.houseID))
		return 0;
	if ((target->o.seenByHouses & (1 << Unit_GetHouseID(unit))) == 0)
		return 0;
...
void Unit_LaunchHouseMissile(const Structure* s, uint16 packed)
{
	...
	houseID = (HouseType)g_unitHouseMissile->o.houseID;

	Unit_Free(g_unitHouseMissile);

	tile32 origin;
	if (s != NULL && s->o.type == STRUCTURE_PALACE)
	{
		const StructureLayout layout = (const StructureLayout)g_table_structureInfo[s->o.type].layout;
		origin = s->o.position;
	...
	Unit_CreateBullet(origin, (UnitType)g_unitHouseMissile->o.type, g_unitHouseMissile->o.houseID, 0x1F4, Tools_Index_Encode(packed, IT_TILE));
...
void Unit_HouseUnitCount_Remove(Unit* unit)
{
	...
	while (true)
	{
		...
		if ((unit->o.seenByHouses & (1 << h->index)) == 0)
			continue;
...
void Unit_HouseUnitCount_Add(Unit* unit, uint8 houseID)
{
	...
	if (unit->o.type != UNIT_SANDWORM)
	{
		for (HouseType ally = HOUSE_HARKONNEN; ally < HOUSE_MAX; ally++)
		{
		...
		if (unit->o.type == UNIT_SANDWORM)
		{
			if (hp->timerSandwormAttack == 0)
			{
				 ...
				 */
				{
					...
					if (!g_gameConfig.hints || (hintsShown & mask) != 0)
