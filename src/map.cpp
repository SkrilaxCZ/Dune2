...
static bool Map_InRangePacked(uint16 packed)
{
...
static bool Map_UpdateWall(uint16 packed)
{
	...
	if (Map_GetLandscapeType(packed) != LST_WALL)
		return false;
...
void Map_MakeExplosion(uint16 type, tile32 position, uint16 hitpoints, uint16 unitOriginEncoded)
{
	...
	if (!s_debugNoExplosionDamage && hitpoints != 0)
	{
		...
		while (true)
		{
			...
			if (us == u)
				continue;
			if (House_AreAllied(Unit_GetHouseID(u), Unit_GetHouseID(us)))
				continue;
			...
			if (t != NULL)
			{
				...
				if (targetInfo->bulletType == UNIT_INVALID)
					t->target = unitOriginEncoded;
				continue;
			}
			...
			if (attack != NULL)
			{
				...
				if (Tile_GetDistancePacked(Tools_Index_GetPackedTile(u->targetAttack), packed) <= ui->fireDistance)
					continue;
			}
 ...
 */
static LandscapeType Map_GetLandscapeType_BySpriteID(uint16 spriteID, bool hasStructure)
{
	int16 spriteOffset;

	if (spriteID == g_builtSlabSpriteID)
		return LST_CONCRETE_SLAB;

	if (spriteID == g_bloomSpriteID || spriteID == g_bloomSpriteID + 1)
		return LST_BLOOM_FIELD;

	if (g_wallSpriteID < spriteID && spriteID < g_wallSpriteID + 75)
		return LST_WALL;
	...
	if (hasStructure)
		return LST_STRUCTURE;

	spriteOffset = spriteID - g_landscapeSpriteID; /* Offset in the landscape icon group. */
	if (spriteOffset < 0 || spriteOffset > 80)
		return LST_ENTIRELY_ROCK;

	return (LandscapeType)_landscapeSpriteMap[spriteOffset];
}
...
uint16 Map_GetLandscapeType(uint16 packed)
{
	...
	if (t->overlaySpriteID == g_wallSpriteID)
		return LST_DESTROYED_WALL;
...
void Map_DeviateArea(uint16 type, tile32 position, uint16 radius, uint8 houseID)
{
	...
	while (true)
	{
		...
		if (u == NULL)
			break;
		if (Tile_GetDistance(position, u->o.position) / 16 >= radius)
			continue;
...
void Map_FillCircleWithSpice(uint16 packed, uint16 radius)
{
	...
	for (i = -radius; i <= radius; i++)
	{
		for (j = -radius; j <= radius; j++)
		{
			...
			if (distance == radius && (Tools_Random_256() & 1) == 0)
				continue;

			if (Map_GetLandscapeType(curPacked) == LST_SPICE)
				continue;
...
static void Map_FixupSpiceEdges(uint16 packed)
{
	...
	if (type == LST_SPICE || type == LST_THICK_SPICE)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			if (type == LST_SPICE)
			{
				if (curType == LST_SPICE || curType == LST_THICK_SPICE)
					spriteID |= (1 << i);
				continue;
			}

			if (curType == LST_THICK_SPICE)
				spriteID |= (1 << i);
		}
...
void Map_ChangeSpiceAmount(uint16 packed, int16 dir)
{
	...
	if (type == LST_THICK_SPICE && dir > 0)
		return;
	if (type != LST_SPICE && type != LST_THICK_SPICE && dir < 0)
		return;
	if (type != LST_NORMAL_SAND && type != LST_ENTIRELY_DUNE && type != LST_SPICE && dir > 0)
		return;
	...
	while (ret == 0)
	{
		...
		case 4:
			{ /* Air */
				...
				if (houseID == g_playerHouseID && !Map_IsValidPosition(ret))
					ret = 0;
				break;
			}
		...
		ret &= 0xFFF;
		if (ret != 0 && Object_GetByPackedTile(ret) != NULL)
			ret = 0;
	}
...
void Map_UpdateAround(uint16 radius, tile32 position, Unit* unit, uint8 function)
{
	...
	if (radius == 0 || (position.x == 0 && position.y == 0))
		return;
	...
	if (radius >= 32)
	{
		...
		for (i = -2; i <= 2; i++)
		{
			for (j = -2; j <= 2; j++)
			{
				...
				switch (function)
				{
				case 0: Map_Update(curPacked, 0, false);
					break;
				case 1: Map_Update(curPacked, 3, false);
					break;
				case 2: Unit_RemoveFromTile(unit, curPacked);
					break;
				case 3: Unit_AddToTile(unit, curPacked);
					break;
				default: break;
				}
			}
	...
	for (y = ymin; y <= ymax; y++)
	{
		for (x = xmin; x <= xmax; x++)
		{
			...
			if (!Map_IsValidPosition(curPacked))
				continue;
			if (g_map[curPacked].hasStructure)
				continue;
			if (Unit_Get_ByPackedTile(curPacked) != NULL)
				continue;
...
static void Map_UnveilTile_Neighbour(uint16 packed)
{
	...
	if (t->isUnveiled)
	{
		...
		if (g_validateStrictIfZero == 0 && Sprite_IsUnveiled(t->overlaySpriteID))
			return;
...
bool Map_UnveilTile(uint16 packed, uint8 houseID)
{
	...
	if (t->isUnveiled && Sprite_IsUnveiled(t->overlaySpriteID))
		return false;
	t->isUnveiled = true;
