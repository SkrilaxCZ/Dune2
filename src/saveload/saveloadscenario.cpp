...
bool Scenario_Load2(FILE* fp, uint32 length)
{
	if (length != HOUSE_MAX)
		return false;
	...
	for (HouseType h = HOUSE_HARKONNEN; h < HOUSE_MAX; h++)
	{
		...
		if (c == ' ')
			...
			g_skirmish.brain[h] = BRAIN_HUMAN;
		else if (c == 'C')
			g_skirmish.brain[h] = BRAIN_CPU_ENEMY;
		else if (c == 'A')
			g_skirmish.brain[h] = BRAIN_CPU_ALLY;
	}

	return true;
}

bool Scenario_Save2(FILE* fp)
{
