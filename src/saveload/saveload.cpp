...
uint32 SaveLoad_GetLength(const SaveLoadDesc* sld)
{
	...
	while (sld->type_disk != SLDT_NULL)
	{
		...
		{
		case SLDT_NULL: length += 0;
			break;
		case SLDT_CALLBACK: length += 0;
			break;
		case SLDT_UINT8: length += sizeof(uint8) * sld->count;
			break;
		case SLDT_UINT16: length += sizeof(uint16) * sld->count;
			break;
		case SLDT_UINT32: length += sizeof(uint32) * sld->count;
			break;
		case SLDT_INT8: length += sizeof(int8) * sld->count;
			break;
		case SLDT_INT16: length += sizeof(int16) * sld->count;
			break;
		case SLDT_INT32: length += sizeof(int32) * sld->count;
			break;
...
bool SaveLoad_Load(const SaveLoadDesc* sld, FILE* fp, void* object)
{
	while (sld->type_disk != SLDT_NULL)
	{
		...
		for (i = 0; i < sld->count; i++)
		{
			...
			case SLDT_UINT8:
				{
					...
					if (fread(&v, sizeof(uint8), 1, fp) != 1)
						return false;
			...
			case SLDT_SLD:
				if (!SaveLoad_Load(sld->sld, fp, ptr))
					return false;
				break;
...
bool SaveLoad_Save(const SaveLoadDesc* sld, FILE* fp, void* object)
{
	while (sld->type_disk != SLDT_NULL)
	{
		...
		for (i = 0; i < sld->count; i++)
		{
			...
			case SLDT_SLD:
				if (!SaveLoad_Save(sld->sld, fp, ptr))
					return false;
				break;
			...
			case SLDT_UINT8:
				{
					...
					if (fwrite(&v, sizeof(uint8), 1, fp) != 1)
						return false;
				}
				break;
			...
			case SLDT_INT8:
				{
					...
					if (fwrite(&v, sizeof(int8), 1, fp) != 1)
						return false;
				}
				break;
			...
			case SLDT_CUSTOM:
				{
					...
					data.fp = fp;
					data.object = object;
					if (sld->callback(&data, 0, false) == 0)
						return false;
				}
				break;
