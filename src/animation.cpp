...
typedef struct Animation
{
	...
	StructureLayout tileLayout; /*!< Tile layout of the Animation. */
	HouseType houseID; /*!< House of the item being animated. */
...
{
	Audio_PlaySoundAtTile((SoundID)parameter, animation->tile);
}
...
void Animation_Start(const AnimationCommandStruct* commands, tile32 tile, uint16 tileLayout, uint8 houseID, uint8 iconGroup)
{
	...
	Animation* animation = (Animation *)BinHeap_Push(&s_animations, Timer_GetTicks());
	if (animation != NULL)
	{
		animation->tileLayout = (StructureLayout)tileLayout;
		animation->houseID = (HouseType)houseID;
		animation->current = 0;
...
void Animation_Stop_ByTile(uint16 packed)
{
	...
	for (int i = 1; i < s_animations.num_elem; i++)
	{
		...
		if (animation->commands == NULL)
			continue;
		if (Tile_PackTile(animation->tile) != packed)
			continue;
...
void Animation_Tick()
{
	...
	Animation* animation = (Animation *)BinHeap_GetMin(&s_animations);
	while ((animation != NULL) && (animation->tickNext <= curr_ticks))
	{
		while (animation->commands != NULL)
		{
			...
			{
			case ANIMATION_STOP:
			default: Animation_Func_Stop(animation, parameter);
				break;

			case ANIMATION_ABORT: Animation_Func_Abort(animation, parameter);
				break;
			case ANIMATION_SET_OVERLAY_SPRITE: Animation_Func_SetOverlaySprite(animation, parameter);
				break;
			case ANIMATION_PAUSE: Animation_Func_Pause(animation, parameter);
				break;
			case ANIMATION_REWIND: Animation_Func_Rewind(animation, parameter);
				break;
			case ANIMATION_PLAY_VOICE: Animation_Func_PlayVoice(animation, parameter);
				break;
			case ANIMATION_SET_GROUND_SPRITE: Animation_Func_SetGroundSprite(animation, parameter);
				break;
			case ANIMATION_FORWARD: Animation_Func_Forward(animation, parameter);
				break;
			case ANIMATION_SET_ICONGROUP: Animation_Func_SetIconGroup(animation, parameter);
				break;
			}
