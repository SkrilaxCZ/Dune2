 ...
 */
int64_t Random_Starport_GetSeedTime()
{
 ...
 */
uint16 Random_Starport_GetSeed(uint16 scenarioID, HouseType houseID)
{
 ...
 */
static int16 Random_Starport_RandomLCG()
{
 ...
 */
static uint16 Random_Starport_Range(uint16 min, uint16 max)
{
	uint16 ret;
	assert(min <= max);
 ...
 * @details Exact.
 */
uint16 Random_Starport_CalculatePrice(uint16 credits)
{
	const uint16 rnd
 ...
 */
uint16 Random_Starport_CalculateUnitPrice(UnitType unitType)
{
	Structure s;
