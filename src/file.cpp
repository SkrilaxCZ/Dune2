...
static unsigned int FileHash_djb2(const char* str)
{
...
FileInfo* FileHash_Store(const char* key)
{
...
unsigned int FileHash_FindIndex(const char* key)
{
	unsigned int idx = FileHash_djb2(key);

	while (s_hash_file[idx].filename != NULL)
	{
		if (stricmp(key, s_hash_file[idx].filename) == 0)
			return idx;
...
static FileInfo* FileHash_Find(const char* key)
{
...
{
	if (putc(value & 0xff, stream) == EOF)
		return false;
	if (putc((value >> 8) & 0xff, stream) == EOF)
		return false;
	if (putc((value >> 16) & 0xff, stream) == EOF)
		return false;
	if (putc((value >> 24) & 0xff, stream) == EOF)
		return false;
	return true;
}
 ...
 */
static uint8 _File_OpenInDir(SearchDirectory dir, const char* filename, uint8 mode)
{
	...
	if ((mode & FILE_MODE_READ_WRITE) == 0)
		return FILE_INVALID;
	...
	if (!fileInfoIndex->flags.isLoaded)
	{
		...
		while (true)
		{
			...
			for (i = 0; i < sizeof(pakFilename); i++)
			{
				...
				if (pakFilename[i] >= 'A' && pakFilename[i] <= 'Z')
					pakFilename[i] += 32;
			}
			...
			if (pakIndex == NULL)
				continue;
			if (pakIndex->parentIndex != fileInfoIndex->parentIndex)
				continue;
...
static uint8 _File_Open(SearchDirectory dir, const char* filename, uint8 mode)
{
	uint8 ret = _File_OpenInDir(dir, filename, mode);
	return ret;
}
...
uint16* File_ReadWholeFileLE16(const char* filename)
{
	...
	buffer = (uint16*)malloc(count * sizeof(uint16));
...
uint32 ChunkFile_Seek(uint8 index, uint32 chunk)
{
	...
	while (true)
	{
		if (File_Read(index, &value, 4) != 4 && !first)
			return 0;

		if (value == 0 && File_Read(index, &value, 4) != 4 && !first)
			return 0;

		if (File_Read(index, &length, 4) != 4 && !first)
			return 0;
...
uint32 ChunkFile_Read(uint8 index, uint32 chunk, void* buffer, uint32 buflen)
{
	...
	while (true)
	{
		...
		if (value == chunk)
		{
			...
			if (buflen < length)
				File_Seek(index, length - buflen, 1);
