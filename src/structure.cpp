...
void GameLoop_Structure()
{
	...
	while (true)
	{
		...
		if (tickStructure)
		{
			...
			{
				uint16 repairCost = 2 * si->o.buildCredits / si->o.hitpoints;
					...
					if (buildCost / 256 <= h->credits)
					{
						...
						else
						{
							...
							if (s->o.houseID == g_playerHouseID)
							{
								if (s->o.type != STRUCTURE_BARRACKS && s->o.type != STRUCTURE_WOR_TROOPER)
								{
									...
									if (s->o.type == STRUCTURE_HIGH_TECH)
										stringID = STR_IS_COMPLETE;
									if (s->o.type == STRUCTURE_CONSTRUCTION_YARD)
										stringID = STR_IS_COMPLETED_AND_READY_TO_PLACE;
							...
							else if (s->o.type == STRUCTURE_CONSTRUCTION_YARD)
							{
								...
								for (i = 0; i < 5; i++)
								{
									if (ns->o.type != h->ai_structureRebuild[i][0])
										continue;

									if (!Structure_Place(ns, h->ai_structureRebuild[i][1], (HouseType)h->index))
										continue;
					...
					if (!s->o.flags.s.onHold && s->countDown != 0 && s->o.linkedID != 0xFFFF)
					{
						...
						if (repairCost < h->credits)
						{
							...
							else
							{
								...
								if (s->o.houseID == g_playerHouseID)
									Audio_PlayVoice((VoiceID)(VOICE_HARKONNEN_VEHICLE_REPAIRED + (int)g_playerHouseID));
							}
						}
					}
					else if (h->credits != 0)
...
uint8 Structure_StringToType(const char* name)
{
	...
	{
		if (stricmp(g_table_structureInfo[type].o.name, name) == 0)
			return type;
	}
...
Structure* Structure_Create(uint16 index, uint8 typeID, uint8 houseID, uint16 position)
{
	...
	if (position != 0xFFFF && !Structure_Place(s, position, (HouseType)houseID))
	{
		Structure_Free(s);
		return NULL;
	}
...
bool Structure_Place(Structure* s, uint16 position, HouseType houseID)
{
	...
	case STRUCTURE_WALL:
		{
			...
			if (s->o.houseID == g_playerHouseID)
				Tile_RemoveFogInRadius(Tile_UnpackTile(position), 1);
	...
	case STRUCTURE_SLAB_2x2:
		{
			...
			for (i = 0; i < g_table_structure_layoutTileCount[si->layout]; i++)
			{
				...
				if (Structure_IsValidBuildLocation(curPos, STRUCTURE_SLAB_1x1) == 0)
					continue;
				...
				if (Map_IsPositionUnveiled(curPos))
					t->overlaySpriteID = 0;
	...
	if (validBuildLocation == 0 && !g_debugScenario && g_validateStrictIfZero == 0)
		return false;
...
void Structure_CalculateHitpointsMax(House* h)
{
	...
	while (true)
	{
		...
		if (s->hitpointsMax >= s->o.hitpoints)
			continue;
...
int16 Structure_IsValidBuildLocation(uint16 position, StructureType type)
{
	...
	if (g_validateStrictIfZero == 0 && isValid && type != STRUCTURE_CONSTRUCTION_YARD && !g_debugScenario)
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			lst = Map_GetLandscapeType(curPos);
			if (lst != LST_CONCRETE_SLAB && lst != LST_WALL)
				continue;
			if (g_map[curPos].houseID != g_playerHouseID)
				continue;
	...
	case HOUSE_WEAPON_MISSILE:
		{
			...
			if (!h->flags.human)
			{
				...
				while (true)
				{
					...
					if (sf == NULL)
						break;
					if (sf->o.type == STRUCTURE_SLAB_1x1 || sf->o.type == STRUCTURE_SLAB_2x2 || sf->o.type == STRUCTURE_WALL)
						continue;

					if (House_AreAllied(s->o.houseID, sf->o.houseID))
						continue;
	...
	case HOUSE_WEAPON_SABOTEUR:
		{
			...
			else if (s->o.houseID == g_playerHouseID)
			{
...
bool Structure_Damage(Structure* s, uint16 damage, uint16 range)
{
	...
	if (s->o.hitpoints == 0)
	{
		...
		if (House_AreAllied(g_playerHouseID, s->o.houseID))
			Audio_PlayVoice((VoiceID)(VOICE_HARKONNEN_STRUCTURE_DESTROYED + (int)s->o.houseID));
		else
...
static bool Structure_UpgradeUnlocksNewUnit(const Structure* s, int level)
{
	const StructureInfo* si = &g_table_structureInfo[s->o.type];
	...
	for (int i = 0; i < 8; i++)
	{
		...
		const UnitType u = (const UnitType)si->buildableUnits[i];
...
bool Structure_IsUpgradable(Structure* s)
{
		...
		si->upgradeCampaign[next_upgrade_level][s->o.houseID] <= g_campaignID + 1)
	{
		...
		h = House_Get_ByIndex(s->o.houseID);
		if ((h->structuresBuilt & g_table_structureInfo[STRUCTURE_ROCKET_TURRET].o.structuresRequired) == g_table_structureInfo[STRUCTURE_ROCKET_TURRET].o.structuresRequired)
			return true;
...
bool Structure_ConnectWall(uint16 position, bool recurse)
{
	...
	tile = &g_map[position];
	if (tile->groundSpriteID == spriteID)
		return false;
...
uint16 Structure_FindFreePosition(Structure* s, bool checkForSpice)
{
	...
	for (j = 0; j < 16; j++ , i = (i + 1) & 0xF)
	{
		...
		if (type == LST_WALL || type == LST_ENTIRELY_MOUNTAIN || type == LST_PARTIAL_MOUNTAIN)
			continue;

		t = &g_map[curPacked];
		if (t->hasUnit || t->hasStructure)
			continue;
...
void Structure_Remove(Structure* s)
{
	...
	for (i = 0; i < 5; i++)
	{
		if (h->ai_structureRebuild[i][0] != 0)
			continue;
...
bool Structure_SetUpgradingState(Structure* s, int8 state, Widget* w)
{
	...
	if (state == -1)
		state = s->o.flags.s.upgrading ? 0 : 1;
	...
	if (state == 0 || s->o.flags.s.upgrading || s->upgradeTimeLeft == 0)
		return ret;
...
bool Structure_SetRepairingState(Structure* s, int8 state, Widget* w)
{
	...
	if (!s->o.flags.s.allocated)
		state = 0;

	if (state == -1)
		state = s->o.flags.s.repairing ? 0 : 1;
	...
	if (state == 0 || s->o.flags.s.repairing || s->o.hitpoints == g_table_structureInfo[s->o.type].o.hitpoints)
		return ret;
...
void Structure_UpdateMap(Structure* s)
{
	...
	for (i = 0; i < layoutSize; i++)
	{
		...
		if (Sprite_IsUnveiled(t->overlaySpriteID))
			t->overlaySpriteID = 0;
...
static uint32 Structure_GetPrerequisites(const StructureInfo* si, HouseType houseID)
{
	 ...
	 */
	for (StructureType prereq = STRUCTURE_PALACE; prereq < STRUCTURE_MAX; prereq++)
	{
...
void Structure_HouseUnderAttack(uint8 houseID)
{
	...
	if (houseID != g_playerHouseID && h->flags.doneFullScaleAttack)
		return;
...
{
	const FactoryWindowItem* pa = (const FactoryWindowItem*)a;
	const FactoryWindowItem* pb = (const FactoryWindowItem*)b;
...
void Structure_InitFactoryItems(const Structure* s)
{
	...
	{
		for (StructureType i = STRUCTURE_SLAB_1x1; i < STRUCTURE_MAX; i++)
		{
