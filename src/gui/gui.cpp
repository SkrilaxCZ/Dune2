...
void GUI_DisplayText(const char* str, int16 importance, ...)
{
	...
	if (scrollInProgress)
	{
		if (buffer[0] != '\0')
		{
			if (stricmp(buffer, displayLine2) != 0 && importance >= line3Importance)
			{
				strncpy(displayLine3, buffer, sizeof(displayLine3));
				line3Importance = importance;
			}
		}
		if (displayTimer > Timer_GetTicks())
			return;
	...
	if (buffer[0] != '\0')
	{
		 ...
		 */
		if (stricmp(buffer, displayLine1) != 0 && stricmp(buffer, displayLine2) != 0 && stricmp(buffer, displayLine3) != 0)
		{
	...
	{
		if (displayLine1[0] == '\0' && displayLine2[0] == '\0')
			return;
	}

	if (line2Importance <= line1Importance && displayTimer >= Timer_GetTicks())
		return;
...
void GUI_DrawChar(unsigned char c, int x, int y)
{
	...
	uint8* screen = (uint8*)GFX_Screen_GetActive();
	...
	if (x >= width || (x + fc->width) > width)
		return;
	if (y >= height || (y + g_fontCurrent->height) > height)
		return;
	...
	for (j = 0; j < fc->usedLines; j++)
	{
		for (i = 0; i < fc->width; i++)
		{
			...
			if (g_colours[data & 0xF] != 0)
				screen[x] = g_colours[data & 0xF];
			x++;
		}
		x += remainingWidth;
	}
...
void GUI_DrawText(const char* string, int16 left, int16 top, uint8 fgColour, uint8 bgColour)
{
	...
	if (top < 0)
		top = 0;
	if (left > TRUE_DISPLAY_WIDTH)
		return;
	if (top > TRUE_DISPLAY_HEIGHT)
		return;
...
void GUI_DrawText_Wrapper(const char* string, int16 left, int16 top, uint8 fgColour, uint8 bgColour, uint16 flags, ...)
{
	...
	if ((arg12low != displayedarg12low && arg12low != 0) || string == NULL)
	{
		switch (arg12low)
		{
		case 1: Font_Select(g_fontNew6p);
			break;
		case 2: Font_Select(g_fontNew8p);
			break;
		default: Font_Select(g_fontNew8p);
			break;
		}
...
void GUI_UpdateProductionStringID()
{
	...
	if (!g_table_structureInfo[s->o.type].o.flags.factory)
	{
		if (s->o.type == STRUCTURE_PALACE)
			g_productionStringID = g_table_houseInfo[s->o.houseID].specialWeapon + 0x29;
		return;
	}
...
uint16 GUI_SplitText(char* str, uint16 maxwidth, char delimiter)
{
	...
	while (*str != '\0')
	{
		...
		while (width < maxwidth && *str != delimiter && *str != '\r' && *str != '\0')
			width += Font_GetCharWidth(*str++);

		if (width >= maxwidth)
		{
			while (*str != 0x20 && *str != delimiter && *str != '\r' && *str != '\0')
				width -= Font_GetCharWidth(*str--);
		}
...
void GUI_DrawSprite(Screen screenID, uint8* sprite, int16 posX, int16 posY, uint16 windowID, uint16 flags, ...)
{
	...
	if ((flags & 0x1000) != 0)
		s_variable_72 = (uint16)va_arg(ap, int);
	...
	buf = (uint8*)GFX_Screen_Get_ByIndex(screenID);
	buf += g_widgetProperties[windowID].xBase;

	if ((flags & 0x4000) == 0)
		posX -= g_widgetProperties[windowID].xBase;
...
void GUI_DrawCredits(int credits, uint16 mode, int x)
{
	...
	if (l_tickCreditsAnimation > Timer_GetTicks() && mode == 0)
		return;
	...
	if (mode == 0 && credits == creditsAnimation && creditsAnimationOffset == 0)
		return;
	...
	if (creditsAnimationOffset < 0 && creditsAnimation == 0)
		creditsAnimationOffset = 0;

	creditsAnimation += creditsAnimationOffset / 8;

	if (creditsAnimationOffset > 0)
		creditsAnimationOffset &= 7;
	if (creditsAnimationOffset < 0)
		creditsAnimationOffset = -((-creditsAnimationOffset) & 7);
...
void GUI_Screen_Copy(int16 xSrc, int16 ySrc, int16 xDst, int16 yDst, int16 width, int16 height, Screen screenSrc, Screen screenDst)
{
	...
	if (xSrc >= SCREEN_WIDTH / 8 || xDst >= SCREEN_WIDTH / 8)
		return;
...
const char* GUI_String_Get_ByIndex(int16 stringID)
{
	...
	case -10:
		onoff = g_enable_music;
		break;

	case -11:
		onoff = g_enable_sound;
		break;
	...
	case -13:
		onoff = g_gameConfig.hints;
		break;

	case -14:
		onoff = g_enable_subtitles;
		break;

	case -50:
		return (g_gameConfig.leftClickOrders) ? "Left-click" : "Right-click";
	case -51:
		return (g_gameConfig.holdControlToZoom) ? "Scroll sidebar" : "Zoom viewport";
	case -52:
		return (g_gameConfig.scrollAlongScreenEdge) ? "Screen" : "Viewport";
	case -53:
		onoff = g_gameConfig.autoScroll;
		break;
...
{
	uint8* screen = (uint8*)GFX_Screen_Get_ByIndex(screenID);
...
static void GUI_HallOfFame_Encode(HallOfFameStruct* data)
{
	...
	for (d = (uint8 *)data , i = 0; i < 128; i++ , d++)
		*d = (*d + i) ^ 0xA7;
}
...
uint16 GUI_HallOfFame_DrawData(HallOfFameStruct* data, bool show)
{
	...
	for (i = 0; i < 8; i++ , offsetY += 11)
	{
		...
		if (data[i].score == 0)
			break;
		if (data[i].rank >= MAX_RANKS)
			break;
		if (data[i].houseID >= HOUSE_MAX)
			break;
...
void GUI_DrawScreen(Screen screenID)
{
	...
	if (g_selectionType == SELECTIONTYPE_MENTAT)
		return;
	if (g_selectionType == SELECTIONTYPE_DEBUG)
		return;
	if (g_selectionType == SELECTIONTYPE_UNKNOWN6)
		return;
	if (g_selectionType == SELECTIONTYPE_INTRO)
		return;
