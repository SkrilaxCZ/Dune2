...
void GUI_Widget_Draw(Widget* w)
{
	...
	case DRAW_MODE_SPRITE:
		Shape_DrawRemap((ShapeID)drawParam.sprite, g_playerHouseID, positionLeft, positionTop, (WindowID)0, 0);
		break;
...
uint16 GUI_Widget_HandleEvents(Widget* w)
{
	...
	if (w != l_widget_last || s_widgetReset)
	{
		...
		if (Input_Test((Scancode)MOUSE_LMB))
			l_widget_button_state |= 0x0200;

		/* Check for right click */
		if (Input_Test((Scancode)MOUSE_RMB))
			l_widget_button_state |= 0x2000;
	}
	...
	/* if (g_var_7097 == 0) */
	{
		...
		{
			if ((key & 0x007F) == MOUSE_RMB)
				buttonStateChange = 0x1000;
			if ((key & 0x007F) == MOUSE_LMB)
				buttonStateChange = 0x0100;
		}
	...
	for (; w != NULL; w = GUI_Widget_GetNext(w))
	{
		...
		if ((key & 0x7F) != 0 && ((key & 0x7F) == w->shortcut || (key & 0x7F) == w->shortcut2) && !(key & SCANCODE_RELEASE))
		{
			...
			buttonState = 0;
			if ((key & 0x7F) == w->shortcut2)
				buttonState = (w->flags.buttonFilterRight) << 12;
			if (buttonState == 0)
				buttonState = (w->flags.buttonFilterLeft) << 8;
...
void GUI_Widget_SetShortcuts(Widget* w)
{
...
Widget* GUI_Widget_Allocate(uint16 index, uint16 shortcut, uint16 offsetX, uint16 offsetY, uint16 spriteID, uint16 stringID)
{
	...
	case 1:
		...
		if (String_Get_ByIndex(stringID) != NULL)
	...
	default:
		...
		w->width = Shape_Width((ShapeID)drawParam1.sprite);
		w->height = Shape_Height((ShapeID)drawParam1.sprite);
		break;
	}
