...
void GUI_Widget_TextButton_Draw(Widget* w)
{
	...
	Video_SetClippingArea(div->scalex * positionX + div->x, div->scaley * positionY + div->y, div->scalex * width, div->scaley * height);
	GUI_DrawText_Wrapper(GUI_String_Get_ByIndex(w->stringID), x, positionY + 2, colour, 0, flags);
...
void GUI_Widget_TextButton2_Draw(Widget* w)
{
	...
	GUI_DrawText_Wrapper(String_Get_ByIndex(stringID), positionX + width / 2, positionY, colour, 0, 0x121);
...
void GUI_Widget_Scrollbar_Draw(Widget* w)
{
	...
	if (scrollbar->drawProc != NULL)
		scrollbar->drawProc(w);
}
...
static uint16 GUI_Widget_ActionPanel_GetActionType(bool forceDraw)
{
	...
	else if (Unit_AnySelected())
	{
		if (g_selectionType == SELECTIONTYPE_TARGET)
		{
			...
			switch (activeAction)
			{
			case ACTION_ATTACK: actionType = 4;
				break; /* Attack */
			case ACTION_MOVE: actionType = 5;
				break; /* Movement */
			default: actionType = 6;
				break; /* Harvest */
			}
		}
		else
...
void GUI_Widget_ActionPanel_Draw(bool forceDraw)
{
	...
	if (actionType > 1)
	{
		...
		switch (actionType)
		{
		case 4: stringID = STR_TARGET;
			break; /* Attack */
		case 5: stringID = STR_MOVEMENT;
			break; /* Movement */
		case 6: stringID = STR_HARVEST;
			break; /* Harvest */
		...
		case 6: /* Harvest */
		case 8: /* House Missile */
			ActionPanel_DrawPortrait(actionType, (ShapeID)spriteID);
			break;
			...
			case 2: /* Unit */
				{
					...
					if (isNotPlayerOwned && o->type != UNIT_HARVESTER)
...
void GUI_Widget_DrawWindow(const WindowDesc* desc)
{
...
void GUI_Widget_Savegame_Draw(uint16 key)
{
