...
static unsigned char GUI_EditBox_ScancodeToChar(Scancode key)
{
 ...
 */
int GUI_EditBox(char* text, uint16 maxLength, uint16 unknown1, Widget* w, uint16 (*tickProc)(), uint16 unknown4)
{
	UNUSED(unknown1);
	...
	*t = '\0';
	...
	while (Input_IsInputAvailable())
	{
		...
		key = GUI_EditBox_ScancodeToChar((Scancode)key);
		if (key == '\0')
			continue;
