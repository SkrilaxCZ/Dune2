...
static void Script_Error(const char* error, ...)
{
	...
	if (g_scriptCurrentUnit != NULL)
		type = l_types[0];
	if (g_scriptCurrentStructure != NULL)
		type = l_types[1];
	if (g_scriptCurrentTeam != NULL)
		type = l_types[2];
	...
	case SCRIPT_BINARY:
		{
			...
			switch (parameter)
			{
			case 0: STACK_PUSH((left && right) ? 1 : 0);
				break; /* left && right */
			case 1: STACK_PUSH((left || right) ? 1 : 0);
				break; /* left || right */
			case 2: STACK_PUSH((left == right) ? 1 : 0);
				break; /* left == right */
			case 3: STACK_PUSH((left != right) ? 1 : 0);
				break; /* left != right */
			case 4: STACK_PUSH((left < right) ? 1 : 0);
				break; /* left <  right */
			case 5: STACK_PUSH((left <= right) ? 1 : 0);
				break; /* left <= right */
			case 6: STACK_PUSH((left > right) ? 1 : 0);
				break; /* left >  right */
			case 7: STACK_PUSH((left >= right) ? 1 : 0);
				break; /* left >= right */
			case 8: STACK_PUSH( left + right );
				break; /* left +  right */
			case 9: STACK_PUSH( left - right );
				break; /* left -  right */
			case 10: STACK_PUSH( left * right );
				break; /* left *  right */
			case 11: STACK_PUSH( left / right );
				break; /* left /  right */
			case 12: STACK_PUSH( left >> right );
				break; /* left >> right */
			case 13: STACK_PUSH( left << right );
				break; /* left << right */
			case 14: STACK_PUSH( left & right );
				break; /* left &  right */
			case 15: STACK_PUSH( left | right );
				break; /* left |  right */
			case 16: STACK_PUSH( left % right );
				break; /* left %  right */
			case 17: STACK_PUSH( left ^ right );
				break; /* left ^  right */
...
uint16 Script_LoadFromFile(const char* filename, ScriptInfo* scriptInfo, const ScriptFunction* functions, uint8* data)
{
	...
	if (length != 0)
	{
		if (data != NULL)
		{
			...
			scriptInfo->text = (uint16*)calloc(1, length);
	...
	if (data != NULL)
	{
		...
		scriptInfo->offsets = (uint16*)calloc(1, length);
	...
	if (data != NULL)
		...
		scriptInfo->start = (uint16 *)calloc(1, length);
