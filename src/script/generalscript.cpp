...
uint16 Script_General_Unknown0288(ScriptEngine* script)
{
	...
	if (s != NULL && Tools_Index_Encode(s->o.index, IT_STRUCTURE) != index)
		return 1;
...
uint16 Script_General_VoicePlay(ScriptEngine* script)
{
	...
	Audio_PlaySoundAtTile((SoundID)STACK_PEEK(1), position);
	return 0;
}
...
uint16 Script_General_IsFriendly(ScriptEngine* script)
{
	...
	if (o == NULL || o->flags.s.isNotOnMap || !o->flags.s.used)
		return 0;
...
uint16 Script_General_IsEnemy(ScriptEngine* script)
{
	...
	{
	case IT_UNIT:
		h2 = (HouseType)Unit_GetHouseID(Tools_Index_GetUnit(index));
		break;
	case IT_STRUCTURE:
		h2 = (HouseType)Tools_Index_GetStructure(index)->o.houseID;
		break;
	default: return 0;
	}
...
uint16 Script_General_FindIdle(ScriptEngine* script)
{
	...
	if (Tools_Index_GetType(index) == IT_UNIT)
		return 0;
	if (Tools_Index_GetType(index) == IT_TILE)
		return 0;
