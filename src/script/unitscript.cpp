...
uint16 Script_Unit_RandomSoldier(ScriptEngine* script)
{
	...
	if (Tools_Random_256() >= g_table_unitInfo[u->o.type].o.spawnChance)
		return 0;
	...
	Unit_SetAction(nu, (ActionType) STACK_PEEK(1));
...
uint16 Script_Unit_GetTargetPriority(ScriptEngine* script)
{
	...
	if (target != NULL)
		return Unit_GetTargetUnitPriority(u, target);
...
uint16 Script_Unit_TransportDeliver(ScriptEngine* script)
{
	...
	if (!Map_IsValidPosition(Tile_PackTile(Tile_Center(u->o.position))))
		return 0;

	u2 = Unit_Get_ByIndex(u->o.linkedID);

	if (!Unit_SetPosition(u2, Tile_Center(u->o.position)))
		return 0;
	...
	case IT_UNIT:
		{
			...
			while (true)
			{
				...
				{
					if (s2->o.type != STRUCTURE_REFINERY || s2->state != STRUCTURE_STATE_IDLE || s2->o.script.variables[4] != 0)
						continue;
				...
				if (s2->o.type != STRUCTURE_REPAIR || s2->state != STRUCTURE_STATE_IDLE || s2->o.script.variables[4] != 0)
					continue;
...
uint16 Script_Unit_SetSpeed(ScriptEngine* script)
{
	...
	if (!u->o.flags.s.byScenario)
		speed = speed * 192 / 256;

	if (g_table_unitInfo[u->o.type].movementType == MOVEMENT_WINGER)
		speed = Tools_AdjustToGameSpeed(speed, 0, 255, true);
...
uint16 Script_Unit_Fire(ScriptEngine* script)
{
	...
	if (u->o.type != UNIT_SANDWORM && target == Tools_Index_Encode(Tile_PackTile(u->o.position), IT_TILE))
		u->targetAttack = 0;
	...
	if (u->o.type != UNIT_SANDWORM && u->orientation[ui->o.flags.hasTurret ? 1 : 0].speed != 0)
		return 0;

	if (Tools_Index_GetType(target) == IT_TILE && Object_GetByPackedTile(Tools_Index_GetPackedTile(target)) != NULL)
		Unit_SetTarget(u, target);
	...
	if ((int16)(ui->fireDistance << 8) < (int16)distance)
		return 0;
	...
	if (fireTwice)
	{
		...
		if (u->o.flags.s.fireTwiceFlip)
			u->fireDelay = Tools_AdjustToGameSpeed(5, 1, 10, true);
	}
	else
...
uint16 Script_Unit_Rotate(ScriptEngine* script)
{
	...
	if (ui->movementType != MOVEMENT_WINGER && (u->currentDestination.x != 0 || u->currentDestination.y != 0))
		return 1;

	index = ui->o.flags.hasTurret ? 1 : 0;

	/* Check if we are already rotating */
	if (u->orientation[index].speed != 0)
		return 1;
...
uint16 Script_Unit_SetAction(ScriptEngine* script)
{
	...
	action = (ActionType)STACK_PEEK(1);

	if (u->o.houseID == g_playerHouseID && action == ACTION_HARVEST && u->nextActionID != ACTION_INVALID)
		return 0;
...
uint16 Script_Unit_GetInfo(ScriptEngine* script)
{
	...
	case 0x06:
		if (u->originEncoded == 0 || u->o.type == UNIT_HARVESTER)
...
static void Script_Unit_Pathfinder_Smoothen(Pathfinder_Data* data)
{
	...
	if (data->routeSize > 1)
	{
		...
		while (*bufferTo != 0xFF)
		{
			...
			while (*bufferFrom == 0xFE && bufferFrom != data->buffer)
				bufferFrom--;
			...
			*bufferFrom = 0xFE;
			...
			/* Walk back one tile */
			while (*bufferFrom == 0xFE && data->buffer != bufferFrom)
				bufferFrom--;
			if (*bufferFrom != 0xFE)
			{
...
static Pathfinder_Data Script_Unit_Pathfinder(uint16 packedSrc, uint16 packedDst, void* buffer, int16 bufferSize)
{
	...
	if (res.routeSize < bufferSize)
		res.buffer[res.routeSize++] = 0xFF;
...
static uint16 Script_Unit_Pathfinder_FindNearbyDestination(Unit* u, uint16 packedSrc, uint16 packedDst)
{
	const struct
	{
		int dx, dy;
	...
	if (dist_src_dest <= (256 * 1))
		return 0;
	else if (dist_src_dest <= (256 * 1.5))
		end = 4;
	else if (dist_src_dest <= (256 * 2))
		end = 8;
	else if (dist_src_dest <= (256 * 2.5))
		end = 12;
	else if (dist_src_dest <= (256 * 3))
		end = 20;
	else
		end = lengthof(offset);
...
uint16 Script_Unit_CalculateRoute(ScriptEngine* script)
{
	...
	if (u->currentDestination.x != 0 || u->currentDestination.y != 0 || !Tools_Index_IsValid(encoded))
		return 1;
...
uint16 Script_Unit_CallUnitByType(ScriptEngine* script)
{
	...
	if (u->o.script.variables[4] != 0)
		return u->o.script.variables[4];
	if (!g_table_unitInfo[u->o.type].o.flags.canBePickedUp || u->deviated != 0)
		return 0;

	encoded = Tools_Index_Encode(u->o.index, IT_UNIT);

	u2 = Unit_CallUnitByType((UnitType)STACK_PEEK(1), Unit_GetHouseID(u), encoded, false);
	if (u2 == NULL)
		return 0;
...
uint16 Script_Unit_Unknown2552(ScriptEngine* script)
{
	...
	if (u2 == NULL || u2->o.type != UNIT_CARRYALL)
		return 0;
...
uint16 Script_Unit_Harvest(ScriptEngine* script)
{
	...
	if (AI_IsBrutalAI((HouseType)Unit_GetHouseID(u)))
	{
...
uint16 Script_Unit_GetRandomTile(ScriptEngine* script)
{
	...
	if (Tools_Index_GetType(STACK_PEEK(1)) != IT_TILE)
		return 0;
...
uint16 Script_Unit_IdleAction(ScriptEngine* script)
{
	...
	if (movementType != MOVEMENT_FOOT && movementType != MOVEMENT_TRACKED && movementType != MOVEMENT_WHEELED)
		return 0;
	...
	case IT_UNIT:
		{
			...
			if (Tools_Index_Encode(u->o.index, IT_UNIT) == u2->o.script.variables[4] && u2->o.houseID == u->o.houseID)
				return 1;
	...
	case IT_STRUCTURE:
		{
			...
			if (Tools_Index_Encode(u->o.index, IT_UNIT) == s->o.script.variables[4] && s->o.houseID == u->o.houseID)
				return 1;
		}
		break;
