...
uint16 Script_Structure_RefineSpice(ScriptEngine* script)
{
	...
	if (u->amount < harvesterStep)
		harvesterStep = u->amount;
	if (u->amount != 0 && harvesterStep < 1)
		harvesterStep = 1;
	if (harvesterStep == 0)
		return 0;
...
uint16 Script_Structure_Unknown0A81(ScriptEngine* script)
{
	...
	if (u != NULL)
	{
		if (structureIndex == u->o.script.variables[4])
			return s->o.script.variables[4];
...
uint16 Script_Structure_FindUnitByType(ScriptEngine* script)
{
	...
	carryall = Unit_CallUnitByType((UnitType)type, s->o.houseID, Tools_Index_Encode(s->o.index, IT_STRUCTURE), position == 0);
...
uint16 Script_Structure_Unknown0C5A(ScriptEngine* script)
{
	...
	if (g_table_unitInfo[u->o.type].movementType == MOVEMENT_WINGER && Unit_SetPosition(u, s->o.position))
	{
		...
		if (s->o.houseID == g_playerHouseID)
			Audio_PlayVoice(VOICE_HARKONNEN_UNIT_LAUNCHED + (int)g_playerHouseID);
	...
	if (s->rallyPoint != 0xFFFF)
	{
		...
		UnitActionType action = ACTION_MOVE;
		...
		if (u->o.type == UNIT_HARVESTER)
		{
			...
			{
				const LandscapeType lst = Map_GetLandscapeTypeVisible(s->rallyPoint);
	...
	Audio_PlayVoice((VoiceID)(g_playerHouseID + (int)((u->o.type == UNIT_HARVESTER) ? VOICE_HARKONNEN_HARVESTER_DEPLOYED : VOICE_HARKONNEN_UNIT_DEPLOYED)));
...
uint16 Script_Structure_VoicePlay(ScriptEngine* script)
{
	...
	Audio_PlaySoundAtTile((SoundID)STACK_PEEK(1), s->o.position);
...
uint16 Script_Structure_Destroy(ScriptEngine* script)
{
	...
	for (i = 0; i < g_table_structure_layoutTileCount[layout]; i++)
	{
		...
		if (g_table_structureInfo[s->o.type].o.spawnChance < Tools_Random_256())
			continue;
