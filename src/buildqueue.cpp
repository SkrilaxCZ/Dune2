...
static BuildQueueItem* BuildQueue_AllocItem(uint16 objectType, int credits)
{
	BuildQueueItem* e = (BuildQueueItem*)malloc(sizeof(*e));
	assert(e);
...
uint16 BuildQueue_RemoveHead(BuildQueue* queue)
{
	BuildQueueItem* e = queue->first;
	uint16 ret = 0xFFFF;
...
bool BuildQueue_RemoveTail(BuildQueue* queue, uint16 objectType, int* credits)
{
...
bool BuildQueue_IsEmpty(const BuildQueue* queue)
{
	return (queue->first == NULL);
}

int BuildQueue_Count(const BuildQueue* queue, uint16 objectType)
{
	BuildQueueItem* e = queue->first;
	int count = 0;
