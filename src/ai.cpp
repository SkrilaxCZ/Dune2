...
ENUM_MATH_OPERATORS(AISquadState)
...
static int UnitAI_CountUnits(HouseType houseID, UnitType unit_type);
...
static bool StructureAI_ShouldBuildCarryalls(HouseType houseID)
{
...
static bool StructureAI_ShouldBuildHarvesters(HouseType houseID)
{
	PoolFindStruct find;
...
static bool StructureAI_ShouldBuildInfantry(HouseType houseID)
{
...
static uint32 StructureAI_FilterBuildOptions(StructureType s, HouseType houseID, uint32 buildable)
{
	switch (s)
	{
...
static uint32 StructureAI_FilterBuildOptions_Original(StructureType s, HouseType houseID, uint32 buildable)
{
	PoolFindStruct find;
...
static int StructureAI_RemapBuildItem(int index, uint16* priority)
{
...
static uint32 StructureAI_GetBuildable(const Structure* s)
{
	uint32 ret = 0;
...
uint16 UnitAI_GetAnyEnemyInRange(const Unit* unit)
{
	...
	HouseType houseID = (HouseType)Unit_GetHouseID(unit);
...
static void UnitAI_SquadPlotWaypoints(AISquad* squad, Unit* unit, uint16 target_encoded)
{
	...
	float dx = (float)(originx - targetx);
	...
	int detourx1 = (int)(targetx + plan->distance1 * cos(theta + plan->angle1 * M_PI / 180.0f));
	int detoury1 = (int)(targety - plan->distance1 * sin(theta + plan->angle1 * M_PI / 180.0f));
	int detourx2 = (int)(targetx + plan->distance2 * cos(theta + plan->angle2 * M_PI / 180.0f));
	int detoury2 = (int)(targety - plan->distance2 * sin(theta + plan->angle2 * M_PI / 180.0f));
	int detourx3 = (int)(targetx + plan->distance3 * cos(theta + plan->angle3 * M_PI / 180.0f));
	int detoury3 = (int)(targety - plan->distance3 * sin(theta + plan->angle3 * M_PI / 180.0f));
	...
	squad->plan = (AISquadPlanID)planID;
...
static Unit* UnitAI_SquadFind(const AISquad* squad, PoolFindStruct* find)
{
	Unit* u = Unit_Find(find);
...
static void UnitAI_AssignSquad(Unit* unit, uint16 destination)
{
	SquadID emptySquadID = SQUADID_INVALID;
	int distance;
	...
	if (Tools_Random_256() & 0x1)
	{
		...
		squad->houseID = (HouseType)unit->o.houseID;
		squad->num_members = 1;
		squad->max_members = 3;
...
static void UnitAI_SquadCharge(AISquad* squad)
{
	PoolFindStruct find;
...
static void UnitAI_DisbandSquad(AISquad* squad)
{
	PoolFindStruct find;
...
static bool UnitAI_SquadIsInFormation(const AISquad* squad)
{
	PoolFindStruct find;
...
static bool UnitAI_SquadIsGathered(const AISquad* squad)
{
	PoolFindStruct find;
	tile32 destination;
...
static void UnitAI_ArrangeBattleFormation(AISquad* squad)
{
...
assert_compile(sizeof(SquadID) == sizeof(uint32));
assert_compile(sizeof(AISquadPlanID) == sizeof(uint32));
assert_compile(sizeof(AISquadState) == sizeof(uint32));
assert_compile(sizeof(HouseType) == sizeof(uint32));

static uint32 SaveLoad_BrutalAI_RecruitmentTimeout(void* object, uint32 value, bool loading)
{
	AISquad* squad = (AISquad *)object;
