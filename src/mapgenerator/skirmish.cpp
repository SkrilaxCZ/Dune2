	...
	for (HouseType h = HOUSE_HARKONNEN; h < HOUSE_MAX; h++)
	{
		...
		if (g_skirmish.brain[h] != BRAIN_NONE)
		{
			for (HouseType h2 = HOUSE_HARKONNEN; h2 < HOUSE_MAX; h2++)
			{
				...
				// Make sure two houses don't have the same sprite color
				if (g_skirmish.brain[h2] != BRAIN_NONE && g_table_houseInfo[h].spriteColor == g_table_houseInfo[h2].spriteColor)
					return false;
			}
		}
...
static int Skirmish_PickRandomIsland(const SkirmishData* sd)
{
...
static uint16 Skirmish_PickRandomLocation(uint16 acceptableLstFlags, uint16 unacceptableLstFlags)
{
...
static void Skirmish_TweakTechTree()
{
...
void Skirmish_Prepare()
{
	Skirmish_TweakTechTree();
}

static void Skirmish_GenGeneral()
{
...
static void Skirmish_GenSpiceBlooms()
{
 ...
 */
static int Skirmish_FindBuildableArea(int island, int x0, int y0, SkirmishData* sd, BuildableTile* buildable)
{
...
static bool Skirmish_IsIslandEnclosed(int start, int end, const SkirmishData* sd)
{
	const int dx[4] = {0, 1, 0, -1};
	const int dy[4] = {-1, 0, 1, 0};
...
static void Skirmish_DivideIsland(HouseType houseID, int island, SkirmishData* sd)
{
	int start = sd->island[island].start;
	const int len = sd->island[island].end - sd->island[island].start;
	BuildableTile* orig = (BuildableTile*)malloc(len * sizeof(orig[0]));
	...
	for (int i = 0; i < len; i++)
	{
		...
		if (area >= 50)
		{
			sd->island = (Island*)realloc(sd->island, (sd->nislands + 1) * sizeof(sd->island[0]));
...
static int Skirmish_BuildOrder_Sorter(const void* a, const void* b)
{
	const SkirmishBuildOrder* pa = (SkirmishBuildOrder*)a;
	const SkirmishBuildOrder* pb = (SkirmishBuildOrder*)b;
...
static void Skirmish_GenUnitsAI(HouseType houseID)
{
	...
	for (int count = 8; count > 0;)
	{
		...
		else
		{
			...
			type = (UnitType)StructureAI_PickNextToBuild(factory[r]);
			if (type == UNIT_INVALID)
				continue;
		}

		const UnitActionType actionType = ((Tools_Random_256() & 0x3) == 0) ? ACTION_AMBUSH : ACTION_AREA_GUARD;
		const tile32 position = Tile_UnpackTile(packed);
...
{
	const HouseType houseID = (g_playerHouseID == HOUSE_FREMEN) ? HOUSE_ATREIDES : HOUSE_FREMEN;
	const UnitType type = UNIT_SANDWORM;
	const UnitActionType actionType = ACTION_AMBUSH;
...
static void Skirmish_GenCHOAM()
{
	...
	if (g_techLevel >= 5)
		g_starportAvailable[UNIT_LAUNCHER] = Tools_RandomLCG_Range(2, 4);
	if (g_techLevel >= 6)
		g_starportAvailable[UNIT_SIEGE_TANK] = Tools_RandomLCG_Range(3, 5);
}
...
bool Skirmish_GenerateMap(bool newseed)
{
	bool generate_houses = Skirmish_IsPlayable();
	assert(g_validateStrictIfZero == 0);
	...
	if (generate_houses)
	{
		sd.island = (Island*)malloc(sizeof(sd.island[0]));
		assert(sd.island != NULL);
	...
	g_generatingMap = true;
	bool ret = Skirmish_GenerateMapInner(generate_houses, &sd);
	g_generatingMap = false;
