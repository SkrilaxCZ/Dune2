...
static FactoryPanelLayout s_factory_panel_layout;

/*--------------------------------------------------------------*/

static void ActionPanel_CalculateOptimalLayout(const Widget* widget, bool is_starport)
{
	...
	{
		s_factory_panel_layout = (is_starport ? FACTORYPANEL_STARPORT_FLAG : FACTORYPANEL_SMALL_ICONS_WITHOUT_SCROLL)
...
static int ActionPanel_ProductionListHeight(const Widget* widget)
{
...
static int ActionPanel_ProductionButtonStride(const Widget* widget, int* ret_items_per_screen)
{
                                   ...
                                   int* x1, int* y1, int* x2, int* y2)
{
	...
	if (y1 != NULL)
		*y1 = y;
	if (x2 != NULL)
		*x2 = x + SCROLL_BUTTON_WIDTH - 1;
	if (y2 != NULL)
		*y2 = y + SCROLL_BUTTON_HEIGHT - 1;
}
                                      ...
                                      int* x1, int* y1, int* x2, int* y2, int* w, int* h)
{
	if (x1 != NULL)
		*x1 = widget->offsetX;
	if (y1 != NULL)
		*y1 = widget->offsetY + widget->height - g_table_gameWidgetInfo[GAME_WIDGET_REPAIR_UPGRADE].height;
	if (x2 != NULL)
		*x2 = widget->offsetX + widget->width - 1;
	if (y2 != NULL)
		*y2 = widget->offsetY + widget->height - 1;
	if (w != NULL)
		*w = widget->width;
	if (h != NULL)
		*h = g_table_gameWidgetInfo[GAME_WIDGET_REPAIR_UPGRADE].height;
}

static void ActionPanel_ClampFactoryScrollOffset(const Widget* widget, Structure* s)
{
	int items_per_screen;
	const int stride = ActionPanel_ProductionButtonStride(widget, &items_per_screen);
...
static bool ActionPanel_ScrollFactory(const Widget* widget, Structure* s)
{
	int delta = g_mouseDZ;
	int x1, y1, x2, y2;
...
void ActionPanel_ClickStarportOrder(Structure* s)
{
	...
	while (!BuildQueue_IsEmpty(&s->queue))
	{
		...
		if (u == NULL)
		{
			 ...
			 */
			h->credits += credits;
			Structure_Starport_Restock((UnitType)objectType);
...
static void ActionPanel_ClickStarportPlus(Structure* s, int entry)
{
...
static void ActionPanel_ClickStarportMinus(Structure* s, int entry)
{
	const FactoryWindowItem* item = &g_factoryWindowItems[entry];
	const uint16 type = item->objectType;
	...
	{
		Structure_Starport_Restock((UnitType)type);
		h->credits += credits;
	}
}
...
static void ActionPanel_DrawStructureLayout(StructureType s, int x1, int y1)
{
	...
	Shape_Draw(SHAPE_STRUCTURE_LAYOUT_OUTLINE, x1 - 1, y1 - 1, (WindowID)0, 0x340);
	...
	for (int ly = 0; ly < lh; ly++)
	{
		...
		{
			Shape_Draw(SHAPE_STRUCTURE_LAYOUT_BLOCK, x1 + size * lx, y1 + size * ly, (WindowID)0, 0);
		}
	}
}

static void ActionPanel_DrawScrollButtons(const Widget* widget)
{
	int items_per_screen;
...
static void ActionPanel_DrawStarportOrder(const Widget* widget, const Structure* s)
{
	if (s_factory_panel_layout == FACTORYPANEL_SMALL_ICONS_WITHOUT_SCROLL)
		return;
...
void ActionPanel_HighlightIcon(HouseType houseID, int x1, int y1, bool large_icon)
{
	...
	switch (g_table_houseInfo[houseID].spriteColor)
	{
...
void ActionPanel_DrawFactory(const Widget* widget, Structure* s)
{
	...
	for (int item = 0; item < g_factoryWindowTotal; item++)
	{
		...
		const ShapeID shapeID = g_factoryWindowItems[item].shapeID;
		...
		{
			Shape_DrawGreyScale(shapeID, x1, y1, w, h, (WindowID)0, 0);
		}
		else if (g_factoryWindowItems[item].available < 0)
		{
			...
			/* Draw layout. */
			if (s->o.type == STRUCTURE_CONSTRUCTION_YARD)
				ActionPanel_DrawStructureLayout((StructureType)object_type, x1, y1);
		}
		else
		{
			Shape_DrawScale(shapeID, x1, y1, w, h, (WindowID)0, 0);
...
void ActionPanel_DrawPalace(const Widget* widget, Structure* s)
{
	const ScreenDiv* div = &g_screenDiv[SCREENDIV_SIDEBAR];
	ShapeID shapeID;
	const char* name;
	...
	case STR_SABOTEUR:
		...
		shapeID = (ShapeID)g_table_unitInfo[type].o.spriteID;
		name = String_Get_ByIndex(g_productionStringID);
		deploy = String_Get_ByIndex(STR_DEPLOY);
		break;
	...
	Shape_DrawScale(shapeID, xcentre - w / 2, y, w, h, (WindowID)0, 0);
