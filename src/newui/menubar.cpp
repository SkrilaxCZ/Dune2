...
void MenuBar_DrawCredits(int credits_new, int credits_old, int offset, int x0)
{
	...
	for (int i = 0; i < 6; i++)
	{
		const ShapeID shape_old = (const ShapeID)((int)SHAPE_CREDITS_NUMBER_0 + char_old[i] - '0');
		const ShapeID shape_new = (const ShapeID)((int)SHAPE_CREDITS_NUMBER_0 + char_new[i] - '0');
		const int x = x0 - digit_w * (6 - i);

		if (char_old[i] != char_new[i])
		{
			if (char_old[i] != ' ')
				Shape_Draw(shape_old, x, y + offset, (WindowID)0, 0);

			if (char_new[i] != ' ')
				Shape_Draw(shape_new, x, y + 8 + offset, (WindowID)0, 0);
		}
		else
		{
			if (char_new[i] != ' ')
				Shape_Draw(shape_new, x, y + 1, (WindowID)0, 0);
		}
	}
}
...
{
	const ScreenDivID divID = A5_SaveTransform();
...
static bool MenuBar_DrawRadarAnimation()
{
...
void MenuBar_Draw(HouseType houseID)
{
	...
	Shape_DrawRemap(SHAPE_CREDITS_LABEL, houseID, menubar->width - 128, 0, (WindowID)0, 0);
...
bool MenuBar_ClickMentat(Widget* w)
{
...
{
	const MentatID mentatID = g_table_houseInfo[g_playerHouseID].mentat;
...
bool MenuBar_ClickOptions(Widget* w)
{
	UNUSED(w);
...
static void MenuBar_DrawGameControlLabel(Widget* w)
{
	const WidgetProperties* wi = &g_widgetProperties[w->parentID];
	GUI_DrawText_Wrapper((const char*)w->data, w->offsetX + wi->xBase - 10, w->offsetY + wi->yBase + 3, 232, 0, 0x222);
}

static bool MenuBar_ClickRadioButton(Widget* radio)
{
...
static bool MenuBar_ClickMusicVolumeSlider(Widget* w)
{
	if (Slider_Click(w))
	{
		const SliderData* data = (const SliderData*)w->data;
...
static bool MenuBar_ClickSoundVolumeSlider(Widget* w)
{
	if (Slider_Click(w) || w->state.buttonState & 0x01)
	{
		const SampleID sampleID = (const SampleID)Tools_RandomLCG_Range(SAMPLE_AFFIRMATIVE, SAMPLE_MOVING_OUT);
		const SliderData* data = (const SliderData*)w->data;
...
static bool MenuBar_ClickScrollSpeedSlider(Widget* w)
{
	if (Slider_Click(w))
	{
		const SliderData* data = (const SliderData*)w->data;
...
static bool MenuBar_ClickPanSensitivitySlider(Widget* w)
{
	if (Slider_Click(w))
	{
		const SliderData* data = (const SliderData*)w->data;
...
static void MenuBar_CreateGameControls()
{
	...
	for (unsigned int i = 0; i < lengthof(slider); i++)
	{
		...
		switch (w->index)
		{
		case 100: data->curr = 20 * music_volume;
			break;
		case 101: data->curr = 20 * sound_volume;
			break;
		case 113: data->curr = clamp(data->min, (g_gameConfig.scrollSpeed / 4), data->max);
			break;
		case 114: data->curr = clamp(data->min, g_gameConfig.panSensitivity / 0.25f, data->max);
			break;
...
static void MenuBar_UninitGameControls()
{
	...
	while (w != NULL)
	{
		...
		if (30 <= w->index && w->index <= 38)
		{ /* Widgets stored in g_table_windowWidgets. */
		}
		else if (w->index >= 100)
		{ /* Sliders. */
			Slider_Free(w);
...
static void MenuBar_TickOptions()
{
...
static void MenuBar_TickSaveLoadGame(GameOverlay overlay)
{
...
static void MenuBar_TickGameControls()
{
	const int widgetID = GUI_Widget_HandleEvents(g_widgetLinkedListTail);
	Widget* w;
	...
	case 0x8000 | 33: g_gameConfig.hints ^= 0x1;
		break;
	case 0x8000 | 34: g_enable_subtitles ^= 0x1;
		break;
	...
	case 0x8000 | 50: g_gameConfig.leftClickOrders ^= 0x1;
		break;
	case 0x8000 | 51: g_gameConfig.holdControlToZoom ^= 0x1;
		break;
	case 0x8000 | 52: g_gameConfig.scrollAlongScreenEdge ^= 0x1;
		break;
	case 0x8000 | 53: g_gameConfig.autoScroll ^= 0x1;
		break;
...
static void MenuBar_TickConfirmation(GameOverlay overlay)
{
	const int widgetID = GUI_Widget_HandleEvents(g_widgetLinkedListTail);
	switch (widgetID)
	{
...
uint16 GUI_DisplayModalMessage(const char* str, uint16 shapeID, ...)
{
	const ScreenDiv* viewport = &g_screenDiv[SCREENDIV_VIEWPORT];
	const ScreenDiv* div = &g_screenDiv[SCREENDIV_MENU];
	const ScreenDivID prev_div = A5_SaveTransform();
	...
	while (true)
	{
		if (redraw)
		{
			...
			{
				Shape_Draw((ShapeID)shapeID, 7, 8, WINDOWID_MODAL_MESSAGE, 0x4000);
