...
ENUM_FLAG_OPERATORS(MenuAction)
...
static void Extras_HideScrollbar();
static void Extras_DrawRadioButton(Widget* w);
static bool Extras_ClickRadioButton(Widget* w);

/*--------------------------------------------------------------*/

static Widget* MainMenu_InitMenuItem(int nth, uint16 index, const char* string)
{
...
static void MainMenu_InitWidgets()
{
	MainMenuWidget menuitem[] =
		{
...
static void PickHouse_InitWidgets()
{
	PickHouseWidget menuitem[] =
		{
			...
			{10, 118, 180, 196 + 5 - 118, 10, SCANCODE_NONE},
		};

	const int width = Font_GetStringWidth("Start level: X");

	menuitem[3].x = (SCREEN_WIDTH - width) / 2 - 6;
	menuitem[4].x = (SCREEN_WIDTH + width) / 2;
	menuitem[5].x = menuitem[3].x;
	menuitem[5].w = menuitem[4].x + 5 - menuitem[3].x;
...
static void Briefing_InitWidgets()
{
	Widget* w;
...
static Widget* Extras_AllocateAndLinkRadioButton(Widget* list, int index, uint16 shortcut, int x, int y)
{
	Widget* w;
...
static void Extras_InitWidgets()
{
	Widget* w;
...
static void Menu_AddCampaign(ALLEGRO_PATH* path)
{
	if (stricmp("skirmish", al_get_path_tail(path)) == 0)
		return;
	...
	char* source = (char*)File_ReadWholeFile_Ex(SEARCHDIR_ABSOLUTE, meta);
	char buffer[120];
	int house[3];
	...
	if (!(HOUSE_HARKONNEN <= house[0] && house[0] < HOUSE_MAX))
		house[0] = HOUSE_INVALID;
	if (!(HOUSE_HARKONNEN <= house[1] && house[1] < HOUSE_MAX))
		house[1] = HOUSE_INVALID;
	if (!(HOUSE_HARKONNEN <= house[2] && house[2] < HOUSE_MAX))
		house[2] = HOUSE_INVALID;
...
static void Menu_ScanCampaigns()
{
	char dirname[1024];
...
static void Menu_FreeWidgets(Widget* w)
{
	while (w != NULL)
	{
		Widget* next = w->next;
...
static void MainMenu_Initialise(Widget* w)
{
	Menu_LoadPalette();
...
static void MainMenu_Draw(Widget* widget)
{
	...
	GUI_DrawText_Wrapper("2.0", SCREEN_WIDTH, SCREEN_HEIGHT - 9, 133, 0, 0x221);
}

static void MainMenu_SetupBlink(Widget* w, int widgetID)
{
	widgetID &= ~0x8000;
...
static bool MainMenu_IsDirty(Widget* w)
{
	while (w != NULL)
	{
...
static MenuAction MainMenu_Loop()
{
	...
	return (redraw ? MENU_REDRAW : MENU_NO_ACTION) | MENU_MAIN_MENU;
}

static bool MainMenu_BlinkLoop(int64_t blink_start)
{
...
static void PickHouse_Initialise()
{
	const int offsetX[3] = { 208, 16, 112 };

	for (HouseType houseID = HOUSE_HARKONNEN; houseID <= HOUSE_ORDOS; houseID++)
	{
		Widget* w = GUI_Widget_Get_ByIndex(pick_house_widgets, houseID);
		w->offsetX = offsetX[houseID];
...
static void PickHouse_Draw()
{
	...
	Shape_DrawTint(SHAPE_CURSOR_LEFT, xl, SCREEN_HEIGHT - 21, fg, (WindowID)0, 0);
	...
	Shape_DrawTint(SHAPE_CURSOR_RIGHT, xr, SCREEN_HEIGHT - 21, fg, (WindowID)0, 0);

	Video_SetClippingArea(0, 0, TRUE_DISPLAY_WIDTH, TRUE_DISPLAY_HEIGHT);
}

static int PickHouse_Loop()
{
	...
	case 0x8000 | HOUSE_MERCENARY:
		g_playerHouseID = (HouseType)(widgetID & (~0x8000));
		g_scenarioID = 1;
		...
		Audio_PlayVoice((VoiceID)(VOICE_HARKONNEN + (int)g_playerHouseID));
...
static void Briefing_Initialise(MenuAction menu, MentatState* mentat)
{
	const MentatID mentatID = (menu == MENU_CONFIRM_HOUSE) ? MENTAT_BENE_GESSERIT : g_table_houseInfo[g_playerHouseID].mentat;
	...
	{
		const BriefingEntry entry =	(menu == MENU_BRIEFING_WIN) ? MENTAT_BRIEFING_WIN :	(menu == MENU_BRIEFING_LOSE) ? 
			MENTAT_BRIEFING_LOSE : MENTAT_BRIEFING_ORDERS;
...
static void Briefing_Draw(MenuAction curr_menu, MentatState* mentat)
{
	const MentatID mentatID = (curr_menu == MENU_CONFIRM_HOUSE) ? MENTAT_BENE_GESSERIT : g_table_houseInfo[g_playerHouseID].mentat;
...
static MenuAction Briefing_Loop(MenuAction curr_menu, HouseType houseID, MentatState* mentat)
{
	...
	if (!Audio_MusicIsPlaying())
	{
		...
		case MENU_BRIEFING_WIN:
			Audio_PlayMusic((MusicID)g_table_houseInfo[houseID].musicWin);
			break;

		case MENU_BRIEFING_LOSE:
			Audio_PlayMusic((MusicID)g_table_houseInfo[houseID].musicLose);
			break;
		...
		default:
			Audio_PlayMusic((MusicID)g_table_houseInfo[houseID].musicBriefing);
			break;
		}
	}
	...
	return (redraw ? MENU_REDRAW : MENU_NO_ACTION) | curr_menu;
}
...
static void BattleSummary_Draw(HouseType houseID, int scenarioID, HallOfFameData* fame)
{
...
static MenuAction BattleSummary_TimerLoop(MenuAction curr_menu, int scenarioID, HallOfFameData* fame)
{
	const int64_t curr_ticks = Timer_GetTicks();
...
static MenuAction BattleSummary_InputLoop(MenuAction curr_menu, HallOfFameData* fame)
{
	switch (fame->state)
	{
	case HALLOFFAME_WAIT_FOR_INPUT:
...
{
	const char* text;
	HouseAnimationType animation;
};

static void PickCutscene_Initialise()
{
	const Cutscene cutscenes[] =
		{
			...
			{"Credits", HOUSEANIMATION_CREDITS},
			{NULL, HOUSEANIMATION_INVALID}
		};

	Widget* w = GUI_Widget_Get_ByIndex(extras_widgets, 3);
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
	ScrollbarItem* si;
...
static MenuAction PickCutscene_Loop(int widgetID)
{
	switch (widgetID)
	{
	case 0x8000 | 1: /* exit. */
...
static MenuAction PlayCutscene_Loop()
{
	...
	case HOUSEANIMATION_LEVEL9_ORDOS:
		Cutscene_PlayAnimation((HouseAnimationType)si->d.offset);
		break;

	case HOUSEANIMATION_CREDITS: /* Credits. */
...
static void PickGallery_Initialise()
{
	Widget* w = GUI_Widget_Get_ByIndex(extras_widgets, 3);
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
...
static void PickGallery_Draw(MentatState* mentat)
{
	GUI_DrawStatusBarText(128, 21);
	MentatBriefing_DrawWSA(mentat);
}

static const ObjectInfo* PickGallery_WSAtoObject(const char* str)
{
	if (str == NULL || str[0] == '\0')
		return NULL;

	for (StructureType s = STRUCTURE_SLAB_1x1; s < STRUCTURE_MAX; s++)
	{
		const ObjectInfo* oi = &g_table_base_structureInfo[s].o;

		if ((oi->wsa != NULL) && (stricmp(str, oi->wsa) == 0))
			return oi;
	}

	for (UnitType u = UNIT_CARRYALL; u < UNIT_MAX; u++)
	{
		const ObjectInfo* oi = &g_table_base_unitInfo[u].o;
...
static MenuAction PickGallery_Loop(MentatState* mentat, int widgetID)
{
	Widget* w = GUI_Widget_Get_ByIndex(extras_widgets, 15);
	...
	if (perform_selection)
	{
		...
		if (mentat->wsa != NULL)
		{
			 ...
			 */
			const ObjectInfo* oi = PickGallery_WSAtoObject((const char*)g_readBuffer);
	...
	return (redraw ? MENU_REDRAW : MENU_NO_ACTION) | MENU_EXTRAS;
}

/*--------------------------------------------------------------*/

static void PickMusic_Initialise()
{
	const struct
	{
		MusicID start, end;
	...
	{
		for (MusicID musicID = category[c].start; musicID <= category[c].end; musicID++)
		{
...
static void PickMusic_Draw(MentatState* mentat)
{
	const ScreenDiv* div = &g_screenDiv[SCREENDIV_MENU];
...
static MenuAction PickMusic_Loop(MentatState* mentat, int widgetID)
{
	...
	case SCANCODE_SPACE:
		...
		const MusicID musicID = (MusicID)(si->d.offset & 0xFF);
...
static void Skirmish_RequestRegeneration(bool force)
{
...
static void Skirmish_Initialise()
{
	Widget* w;
...
static MenuAction Skirmish_Loop(int widgetID)
{
	switch (widgetID)
	{
	case 0x8000 | 1: /* exit. */
...
static void Options_Initialise()
{
	Widget* w = GUI_Widget_Get_ByIndex(extras_widgets, 3);
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
	ScrollbarItem* si;
...
static MenuAction Options_Loop(int widgetID)
{
	Widget* scrollbar = GUI_Widget_Get_ByIndex(extras_widgets, 15);
	ScrollbarItem* si;
...
static void Extras_Initialise(MentatState* mentat)
{
	extras_credits = 0;
	mentat->desc_timer = Timer_GetTicks();
	...
	const ExtrasMenu new_extras_page = extras_page;
...
static void Extras_DrawRadioButton(Widget* w)
{
	const ExtrasMenu page = (const ExtrasMenu)(w->index - 20);
	const ShapeID shapeID[EXTRASMENU_MAX] = {
	...
	if (page == EXTRASMENU_JUKEBOX && !g_enable_audio)
		Shape_DrawGrey(shapeID[page], w->offsetX, w->offsetY, (WindowID)0, 0);
	else
		Shape_Draw(shapeID[page], w->offsetX, w->offsetY, (WindowID)0, 0);
...
static bool Extras_ClickRadioButton(Widget* w)
{
	const ExtrasMenu new_extras_page = (const ExtrasMenu)(w->index - 20);
...
static MenuAction Extras_Loop(MentatState* mentat)
{
	static ScrollbarItem* last_si;
	static int64_t l_pause;

	const int64_t curr_ticks = Timer_GetTicks();
	MenuAction res = MENU_EXTRAS;
	bool redraw = false;
	...
	return (redraw ? MENU_REDRAW : MENU_NO_ACTION) | res;
}

/*--------------------------------------------------------------*/

static MenuAction StrategicMap_InputLoop(int campaignID, StrategicMapData* map)
{
...
static void Menu_DrawFadeIn(int64_t fade_start)
{
...
static void Menu_DrawFadeOut(int64_t fade_start)
{
...
void Menu_Run()
{
	...
	MenuAction next_menu = curr_menu;
	...
	while (curr_menu != MENU_EXIT_GAME)
	{
		...
		if (initialise_menu)
		{
			...
			case MENU_BRIEFING_WIN:
			case MENU_BRIEFING_LOSE:
				Briefing_Initialise((MenuAction)(curr_menu & 0xFF), &g_mentat_state);
				break;
		...
		if (redraw && (last_redraw_time < Timer_GetTicks()))
		{
			...
			case MENU_BRIEFING_WIN:
			case MENU_BRIEFING_LOSE:
				Briefing_Draw((MenuAction)(curr_menu & 0xFF), &g_mentat_state);
				break;
		...
		if ((curr_menu & 0xFF) != (res & 0xFF))
		{
			...
			{
				curr_menu = (MenuAction)(res & 0xFF);
				initialise_menu = true;
			}
			else if (res & MENU_FADE_IN)
			{
				curr_menu = (MenuAction)(res & (MENU_FADE_IN | 0xFF));
			...
			else
			{
				...
				next_menu = (MenuAction)(MENU_FADE_IN | (res & 0xFF));
				fade_start = Timer_GetTicks();
			}
		}
