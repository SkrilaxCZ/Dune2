...
uint16 StrategicMap_CampaignChoiceToScenarioID(int campaignID, int nth)
{
	if (campaignID == 0)
		return 1;
...
static void StrategicMap_SetRegionAttempted(int region)
{
	g_strategicRegionBits |= (1 << region);
}

static bool StrategicMap_IsRegionAttempted(int region)
{
...
static void StrategicMap_DrawPlanet(const StrategicMapData* map)
{
...
static void StrategicMap_DrawEmblem(unsigned char emblemID)
{
	const struct
	{
		int x, y;
...
static void StrategicMap_DrawText(const StrategicMapData* map)
{
...
static void StrategicMap_DrawRegion(HouseType houseID, int region)
{
	Shape_DrawRemap((ShapeID)(SHAPE_MAP_PIECE + region), houseID, region_data[region].x, region_data[region].y, (WindowID)0, 0);
}

static void StrategicMap_DrawRegions(const StrategicMapData* map)
{
	for (int region = 1; region <= STRATEGIC_MAP_MAX_REGIONS; region++)
	{
		const HouseType houseID = map->owner[region];
...
static void StrategicMap_DrawRegionFadeIn(const StrategicMapData* map)
{
	if (map->region_aux != NULL)
		Video_DrawFadeIn(map->region_aux);
}

static void StrategicMap_DrawArrow(HouseType houseID, int scenario, const StrategicMapData* map)
{
	...
	const ShapeID shapeID = map->arrow[scenario].shapeID;
	...
	Shape_DrawTint(tintID + 4, x, y, c + ((frame + 3) & 0x3), (WindowID)0, 0);
}

static void StrategicMap_DrawArrows(HouseType houseID, int blink_scenario, const StrategicMapData* map)
{
...
static bool StrategicMap_GetNextProgression(StrategicMapData* map)
{
...
void StrategicMap_Draw(HouseType houseID, StrategicMapData* map, int64_t fade_start)
{
...
int StrategicMap_SelectRegion(const StrategicMapData* map, int x, int y)
{
	x = x - 8;
	y = y - 24;
...
bool StrategicMap_BlinkLoop(StrategicMapData* map, int64_t blink_start)
{
