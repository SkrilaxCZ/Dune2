...
static Widget* ScrollListArea_Allocate(Widget* scrollbar, WindowID parentID, int x, int y);

/*--------------------------------------------------------------*/

static void GUI_Widget_Scrollbar_CalculateSize(WidgetScrollbar* scrollbar)
{
	Widget* w;
	uint16 size;
...
static void GUI_Widget_Scrollbar_CalculatePosition(WidgetScrollbar* scrollbar)
{
	Widget* w;
	uint16 position;
	...
	if (position != 0)
		position = scrollbar->scrollPosition * (max(w->width, w->height) - 2 - scrollbar->size) / position;
...
static void GUI_Widget_Scrollbar_CalculateScrollPosition(WidgetScrollbar* scrollbar)
{
	Widget* w;
...
static Widget* GUI_Widget_Allocate_WithScrollbar(uint16 index, WindowID parentID,
...
static Widget* GUI_Widget_Allocate3(uint16 index, WindowID parentID, uint16 offsetX, uint16 offsetY,
                                    uint16 sprite1, uint16 sprite2, Widget* widget2, uint16 unknown1A)
{
	...
	w->width = Shape_Width((ShapeID)sprite1);
	w->height = Shape_Height((ShapeID)sprite1);
...
void GUI_Widget_Scrollbar_Init(Widget* w, int16 scrollMax, int16 scrollPageSize, int16 scrollPosition)
{
	...
	if (scrollPageSize >= 0)
		scrollbar->scrollPageSize = min(scrollPageSize, scrollbar->scrollMax);
	if (scrollPosition >= 0)
		scrollbar->scrollPosition = min(scrollPosition, scrollbar->scrollMax - scrollbar->scrollPageSize);
...
ScrollbarItem* Scrollbar_AllocItem(Widget* w, ScrollbarItemType type)
{
	...
	if (s_scrollbar_max_items <= i + 1)
	{
		...
		s_scrollbar_item = (ScrollbarItem*)realloc(s_scrollbar_item, new_max * sizeof(s_scrollbar_item[0]));
...
static int Scrollbar_Sorter(const void* a, const void* b)
{
	const ScrollbarItem* sa = (const ScrollbarItem*)a;
	const ScrollbarItem* sb = (const ScrollbarItem*)b;
	const int ia = atoi(sa->text + 5);
	const int ib = atoi(sb->text + 5);

	if (ia == ib)
	{
		return stricmp(sb->text, sa->text);
	}
	else
	{
		return ib - ia;
...
{
	const WidgetScrollbar* ws = (const WidgetScrollbar*)w->data;
...
static void Scrollbar_Scroll(WidgetScrollbar* scrollbar, uint16 scroll)
{
	...
	if ((int16)scrollbar->scrollPosition <= 0)
		scrollbar->scrollPosition = 0;
...
static void Scrollbar_SelectUp(Widget* w)
{
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
...
static void Scrollbar_SelectDown(Widget* w)
{
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
...
bool Scrollbar_Click(Widget* w)
{
	...
	positionX = w->offsetX;
	if (w->offsetX < 0)
		positionX += g_widgetProperties[w->parentID].width;
	positionX += g_widgetProperties[w->parentID].xBase;

	positionY = w->offsetY;
	if (w->offsetY < 0)
		positionY += g_widgetProperties[w->parentID].height;
...
static void ScrollListArea_Draw(Widget* w)
{
...
static bool ScrollListArea_Click(Widget* w)
{
	const WidgetProperties* wi = &g_widgetProperties[w->parentID];
	WidgetScrollbar* ws = (WidgetScrollbar*)w->data;
