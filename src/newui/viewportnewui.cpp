...
static ViewportClickAction viewport_click_action = VIEWPORT_CLICK_NONE;
...
static bool Map_InRange(int xy)
{
	return (0 <= xy && xy < MAP_SIZE_MAX);
}

static int Viewport_ClampSelectionBoxX(int x)
{
...
static int Viewport_ClampSelectionBoxY(int y)
{
	const WidgetInfo* wi = &g_table_gameWidgetInfo[GAME_WIDGET_VIEWPORT];
...
static SelectionMode Viewport_GetSelectionMode()
{
	if (Unit_AnySelected())
	{
...
static void Viewport_SelectRegion()
{
	const int radius = 5;
	...
	const SelectionMode mode = Viewport_GetSelectionMode();
...
static void Viewport_Target(Unit* u, UnitActionType action, bool command_button, uint16 packed)
{
	uint16 encoded;
	Unit* target = NULL;
	...
	if (g_table_unitInfo[u->o.type].movementType == MOVEMENT_FOOT)
		Audio_PlaySample((SampleID)g_table_actionInfo[action].soundID, 255, 0.0);
	else
	{
		const SampleID sampleID = (((Tools_Random_256() & 0x1) == 0) ? SAMPLE_ACKNOWLEDGED : SAMPLE_AFFIRMATIVE);
		Audio_PlaySample(sampleID, 255, 0.0);
	}
}

static void Viewport_Place()
{
...
static bool Viewport_MouseInScrollWidget()
{
	for (GameWidgetType w = GAME_WIDGET_SCROLL_UP; w <= GAME_WIDGET_SCROLL_DOWN; w++)
	{
...
static void Viewport_ScrollMap(ShapeID* cursorID)
{
	const WidgetInfo* wi;
	int dx = 0, dy = 0;
	...
	if (dx < 0)
		*cursorID = SHAPE_CURSOR_LEFT;
	else if (dx > 0)
		*cursorID = SHAPE_CURSOR_RIGHT;

	if (dy < 0)
		*cursorID = SHAPE_CURSOR_UP;
	else if (dy > 0)
		*cursorID = SHAPE_CURSOR_DOWN;
}

static void Viewport_StopMousePanning()
{
	viewport_click_action = VIEWPORT_CLICK_NONE;
...
static bool Viewport_GenericCommandCanAttack(uint16 packed, LandscapeType lst, bool visible, bool scouted)
{
	if (!scouted)
		return false;
...
static bool Viewport_GenericCommandCanSabotageAlly(uint16 packed)
{
	int iter;
...
static bool Viewport_LMBAttemptsGenericCommand(uint16 packed)
{
	...
	if (s != NULL)
		houseID = (HouseType)s->o.houseID;

	if (houseID == HOUSE_INVALID)
	{
		Unit* u = Unit_Get_ByPackedTile(packed);
		if (u != NULL)
			houseID = (HouseType)Unit_GetHouseID(u);
	}

	return !House_AreAllied(houseID, g_playerHouseID);
}

static bool Viewport_PerformContextSensitiveAction(uint16 packed, bool dry_run)
{
	const LandscapeType lst = Map_GetLandscapeTypeVisible(packed);
	...
	{
		const ObjectInfo* oi = &g_table_unitInfo[u->o.type].o;
		UnitActionType action = ACTION_INVALID;
		...
		for (int i = 0; (i < 4) && (action == ACTION_INVALID); i++)
		{
			...
			else if ((oi->actionsPlayer[i] == ACTION_SABOTAGE) && (lst != LST_BLOOM_FIELD))
			{
				if (attack || sabotage)
...
bool Viewport_Click(Widget* w)
{
	...
	do
	{
		if (w->index != 43)
			break;
		if ((cursorID == SHAPE_CURSOR_TARGET) || (g_timerGame - l_tickCursor <= 10))
			break;
	...
	if (w->state.buttonState & 0x01)
	{
		...
		if ((g_selectionType == SELECTIONTYPE_TARGET) && !mouse_in_scroll_widget)
		{
			...
			for (Unit* u = Unit_FirstSelected(&iter); u; u = Unit_NextSelected(&iter))
				Viewport_Target(u, (UnitActionType)g_activeAction, true, packed);
			...
			if (g_gameConfig.leftClickOrders)
			{
				...
				else
				{
					...
					if ((s != NULL) && !g_map[packed].hasUnit && !g_map[packed].hasStructure &&
						Structure_SupportsRallyPoints((StructureType)s->o.type))
					{
						perform_context_sensitive_action = true;
					}
				}
...
static bool Viewport_TileIsDebris(uint16 iconID)
{
	/* 3x2, 3x3. */
...
void Viewport_DrawRallyPoint()
{
	...
	if (Structure_SupportsRallyPoints((StructureType)s->o.type))
	{
		...
		Shape_DrawTint(SHAPE_CURSOR_TARGET, x2, y2, 14, (WindowID)0, 0x8000);
	}
}

static void Viewport_DrawHealthBar(int x, int y, int width, int curr, int max)
{
...
static void Viewport_DrawSpiceBricks(int x, int y, int num_bricks, int curr, int max)
{
	if (max < num_bricks)
		return;
	...
	if (curr < max)
	{
		...
		if (w > deltax)
			Prim_FillRect(x, y, x + w - deltax, y + 1.0f, 83);
...
static void Viewport_DrawSelectedUnit(int x, int y)
{
...
static void Viewport_DrawUnitHarvesting(const Unit* u, uint8 orientation, int x, int y)
{
	...
	ShapeID shapeID = (ShapeID)((u->spriteOffset % 3) + 0xDF + (values_32A4[orientation][0] * 3));
	uint16 spriteFlags = values_32A4[orientation][1];

	x += values_334E[orientation][0];
	y += values_334E[orientation][1];

	Shape_Draw(shapeID, x, y, (WindowID)0, spriteFlags | 0x8000);
}

static void Viewport_DrawUnitTurret(const Unit* u, const UnitInfo* ui, int x, int y)
{
	...
	ShapeID shapeID = (ShapeID)(ui->turretSpriteID + values_32A4[orientation][0]);
	uint16 spriteFlags = values_32A4[orientation][1];
	...
	Shape_DrawRemap(shapeID, (HouseType)Unit_GetHouseID(u), x, y, (WindowID)0, spriteFlags | 0xA000);
}
...
void Viewport_DrawUnit(const Unit* u, int windowX, int windowY, bool render_for_blur_effect)
{
	...
	if (u->o.type != UNIT_SANDWORM && u->o.flags.s.isHighlighted)
		s_spriteFlags |= 0x100;
	if (ui->o.flags.blurTile)
		s_spriteFlags |= 0x200 | ((u->wobbleIndex & 0x7) << 4);

	Shape_DrawRemap((ShapeID)index, (HouseType)Unit_GetHouseID(u), x, y, (WindowID)0, s_spriteFlags | 0xA000);
	...
	{
		ShapeID shapeID = (ShapeID)(180 + (u->spriteOffset & 3));
		if (shapeID == (ShapeID)183)
			shapeID = (ShapeID)181;

		Shape_Draw(shapeID, x, y - 14, (WindowID)0, 0x8000);
	}
...
void Viewport_DrawAirUnit(const Unit* u)
{
	...
	const UnitInfo* ui = &g_table_unitInfo[u->o.type];
	const bool smooth_rotation = (ui->displayMode == DISPLAYMODE_UNIT || ui->displayMode == DISPLAYMODE_ROCKET || ui->displayMode == DISPLAYMODE_ORNITHOPTER);
	...
	if (ui->o.flags.hasShadow)
	{
		...
		if (smooth_rotation)
			Shape_DrawRemapRotate((ShapeID)index, (HouseType)Unit_GetHouseID(u), x + 1, y + 3, &u->orientation[0], WINDOWID_VIEWPORT, (s_spriteFlags & 0x5FFF) | alpha);
		else
			Shape_Draw((ShapeID)index, x + 1, y + 3, WINDOWID_VIEWPORT, (s_spriteFlags & 0xDFFF) | alpha);
	}
	...
	if (smooth_rotation)
	{
		Shape_DrawRemapRotate((ShapeID)index, (HouseType)Unit_GetHouseID(u), x, y, &u->orientation[0], WINDOWID_VIEWPORT, (s_spriteFlags & 0x7FFF) | 0x2000);
	}
	else
	{
		Shape_DrawRemap((ShapeID)index, (HouseType)Unit_GetHouseID(u), x, y, WINDOWID_VIEWPORT, s_spriteFlags | 0x2000);
	}
}
...
void Viewport_DrawPanCursor()
{
	...
	Shape_Draw(SHAPE_CURSOR_UP, x - 5, y - 13, (WindowID)0, 0);
	Shape_Draw(SHAPE_CURSOR_RIGHT, x + 5, y - 5, (WindowID)0, 0);
	Shape_Draw(SHAPE_CURSOR_DOWN, x - 5, y + 5, (WindowID)0, 0);
	Shape_Draw(SHAPE_CURSOR_LEFT, x - 13, y - 5, (WindowID)0, 0);
	A5_UseTransform(SCREENDIV_VIEWPORT);
}

static void Viewport_DrawInterface(HouseType houseID, int x, int blurx)
{
