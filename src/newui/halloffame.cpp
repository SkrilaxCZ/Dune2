...
int HallOfFame_GetRank(int score)
{
...
const char* HallOfFame_GetRankString(int rank)
{
	return String_Get_ByIndex(rank_scores[rank].stringID);
}

static void HallOfFame_DrawEmblem(unsigned char emblemL, unsigned char emblemR)
{
	int x;
...
void HallOfFame_DrawBackground(HouseType houseID, bool hallOfFame)
{
	...
	{
		const int emblemID = g_table_houseInfo[houseID].fameCps;
...
static void HallOfFame_DrawYouEnemyLabel(int y)
{
...
static void HallOfFame_DrawMeter(HouseType houseID, const HallOfFameData* fame, int meter)
{
	...
	if (w > 0)
	{
		if (ally)
		{
			...
			const int idx =	(houseID == HOUSE_ATREIDES) ? 2 : (houseID == HOUSE_ORDOS) ? 1 : 0;
	...
	const bool done = (meter < fame->curr_meter_idx) ||	(meter == fame->curr_meter_idx && fame->state == HALLOFFAME_PAUSE_METER);
