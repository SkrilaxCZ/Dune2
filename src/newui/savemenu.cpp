...
static bool SaveMenu_IsValidFilename(const ALLEGRO_PATH* path)
{
	const char* extension = al_get_path_extension(path);
	if (stricmp(extension, ".DAT") != 0)
		return false;

	const char* basename = al_get_path_basename(path);
	if (strnicmp(basename, "_SAVE", 5) != 0)
		return false;
...
static void SaveMenu_FillSavegameDesc(bool save, Widget* scrollbar)
{
	WidgetScrollbar* ws = (WidgetScrollbar*)scrollbar->data;
...
static void SaveMenu_FreeScrollbar()
{
...
int SaveMenu_Savegame_Click(uint16 key)
{
	...
	const WidgetScrollbar* ws;
	const ScrollbarItem* si;
	int entry;
	...
	case 0x1E:
		...
		ws = (const WidgetScrollbar*)scrollbar->data;
		entry = ws->scrollPosition + key;
		si = Scrollbar_GetItem(scrollbar, entry);
...
static void SaveMenu_UpdateArrows(Widget* scrollbar)
{
	WidgetScrollbar* ws = (WidgetScrollbar*)scrollbar->data;
	Widget* w;
