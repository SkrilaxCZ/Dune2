...
GraphicsDriver g_graphics_driver;
 ...
 */
static void VideoA5_SetBitmapFlags(int flags)
{
...
static ALLEGRO_BITMAP* VideoA5_ConvertToVideoBitmap(ALLEGRO_BITMAP* membmp)
{
...
static void VideoA5_ResizeScratchBitmap(int w, int h)
{
	if (scratch == NULL)
	{
...
static void VideoA5_ReadPalette(const char* filename)
{
...
static ALLEGRO_BITMAP* VideoA5_InitDisplayIcon(char** xpm, int w, int h, int colours)
{
	struct
	{
		unsigned char r, g, b;
...
static void VideoA5_InitWindowIcons()
{
...
static void VideoA5_UninitCPSStore()
{
	while (s_cps != NULL)
	{
...
void VideoA5_Uninit()
{
	...
	{
		for (ShapeID shapeID = SHAPE_CURSOR_NORMAL; shapeID < SHAPEID_MAX; shapeID++)
		{
...
static void VideoA5_CopyBitmap(int src_stride, const unsigned char* raw, ALLEGRO_BITMAP* dest, BitmapCopyMode mode)
{
	ALLEGRO_LOCKED_REGION* reg;
...
void Video_SetCursor(int spriteID)
{
...
{
	const ScreenDivID prev_transform = A5_SaveTransform();
...
static void VideoA5_InitDissolve_GLStencil(ALLEGRO_BITMAP* src, FadeInAux* aux)
{
...
static void VideoA5_DrawDissolve_GLStencil(const FadeInAux* aux)
{
	if (aux->bmp == NULL)
		return;
...
static void VideoA5_TickDissolve_GLStencil(FadeInAux* aux)
{
	glEnable(GL_STENCIL_TEST);
...
static void VideoA5_InitDissolve_D3DStencil(ALLEGRO_BITMAP* src, FadeInAux* aux)
{
...
static void VideoA5_DrawDissolve_D3DStencil(const FadeInAux* aux)
{
	if (aux->bmp == NULL)
		return;
...
static void VideoA5_TickDissolve_D3DStencil(FadeInAux* aux)
{
	LPDIRECT3DDEVICE9 pDevice = al_get_d3d_device(display);
	if (pDevice == NULL)
		return;
...
static FadeInAux* VideoA5_InitFadeInSprite(ALLEGRO_BITMAP* src, int x, int y, int w, int h, bool fade_in)
{
...
void Video_DrawFadeIn(const FadeInAux* aux)
{
	switch (g_graphics_driver)
	{
	case GRAPHICS_DRIVER_OPENGL:
...
bool Video_TickFadeIn(FadeInAux* aux)
{
	if (aux == NULL)
		return true;
...
static CPSStore* VideoA5_ExportCPS(SearchDirectory dir, const char* filename, unsigned char* buf)
{
	CPSStore* cps = (CPSStore*)malloc(sizeof(*cps));
	if (cps == NULL)
		return NULL;
...
static CPSStore* VideoA5_LoadCPS(SearchDirectory dir, const char* filename)
{
	char campname[1024];
	CPSStore* cps;
	...
	cps = VideoA5_ExportCPS(dir, filename, (unsigned char*)GFX_Screen_Get_ByIndex(SCREEN_1));
	if (cps == NULL)
		return NULL;
...
static void VideoA5_FreeCPS(CPSStore* cps)
{
...
static void VideoA5_InitCPS()
{
	const struct CPSSpecialCoord* coord;
	unsigned char* buf = (unsigned char*)GFX_Screen_Get_ByIndex(SCREEN_1);
	...
	for (CPSID cpsID = CPS_MENUBAR_LEFT; cpsID <= CPS_STATUSBAR_RIGHT; cpsID++)
	{
...
static int VideoA5_NumIconsInGroup(IconMapEntries group)
{
...
static IconConnectivity* VideoA5_CreateIconConnectivities()
{
	IconConnectivity* connect = (IconConnectivity*)calloc(ICONID_MAX, sizeof(connect[0]));
	assert(connect != NULL);

	for (StructureType s = STRUCTURE_PALACE; s < STRUCTURE_MAX; s++)
	{
		const StructureLayout layout = (const StructureLayout)g_table_structureInfo[s].layout;
		const IconMapEntries group = (const IconMapEntries)g_table_structureInfo[s].iconGroup;
		...
		for (int idx0 = 0; idx0 < count; idx0 += w * h)
		{
			for (int i = 0; i < h; i++)
			{
				for (int j = 0; j < w; j++)
				{
					...
					if (i > 0)
						connect[iconID].iconU = g_iconMap[g_iconMap[group] + idx0 + (w * (i - 1) + j)];
					if (i + 1 < h)
						connect[iconID].iconD = g_iconMap[g_iconMap[group] + idx0 + (w * (i + 1) + j)];
					if (j > 0)
						connect[iconID].iconL = g_iconMap[g_iconMap[group] + idx0 + (w * i + (j - 1))];
					if (j + 1 < w)
						connect[iconID].iconR = g_iconMap[g_iconMap[group] + idx0 + (w * i + (j + 1))];
				}
			}
		}
	}
...
static void VideoA5_DrawIconPadding(ALLEGRO_BITMAP* membmp, IconConnectivity* connect)
{
                              ...
                              int x, int y, int* retx, int* rety)
{
	...
	GFX_DrawSprite(iconID, x, y, HOUSE_HARKONNEN);
...
static void VideoA5_CreateInvalidPlacementMask(int x1, int y1)
{
	for (StructureLayout layout = STRUCTURE_LAYOUT_1x1; layout <= STRUCTURE_LAYOUT_3x3; layout++)
	{
		const int w = g_table_structure_layoutSize[layout].width;
		const int h = g_table_structure_layoutSize[layout].height;
...
static void VideoA5_MaskDebrisTiles(ALLEGRO_BITMAP* membmp)
{
	const struct
	{
		int maskx, masky;
		IconMapEntries group;
		int idx;
	} rubble[1 + 4 + 4 + 9] = {
...
static void VideoA5_InitIcons(unsigned char* buf)
{
	const struct
	{
		int num_common;
...
static void VideoA5_InitShapeCHOAMButtons(unsigned char* buf, int y1)
{
	const struct
	{
		int sx, dx;
	...
	for (HouseType houseID = HOUSE_HARKONNEN + 1; houseID < HOUSE_MAX; houseID++)
	{
...
static void VideoA5_InitShapes(unsigned char* buf)
{
	/* Check Sprites_Init. */
	const struct
	{
	...
	for (ShapeID shapeID = SHAPE_ARROW; shapeID <= SHAPE_ARROW_FINAL; shapeID++)
	{
...
/* Requires OpenGL, stencil buffer. */
static void VideoA5_DrawBlur_GLStencil(ALLEGRO_BITMAP* brush, int x, int y, int blurx)
{
...
/* Requires Direct3D, stencil buffer. */
static void VideoA5_DrawBlur_D3DStencil(ALLEGRO_BITMAP* brush, int x, int y, int blurx)
{
	LPDIRECT3DDEVICE9 pDevice = al_get_d3d_device(display);
...
FadeInAux* Video_InitFadeInShape(ShapeID shapeID, HouseType houseID, int x, int y)
{
	assert(shapeID < SHAPEID_MAX);
	assert(houseID < HOUSE_MAX);
	assert(s_shape[shapeID][houseID] != NULL);
...
static int VideoA5_FontIndex(const Font* font, const uint8* pal)
{
	if (font == g_fontNew6p)
	{
		/* 4 colour font. */
		if (memcmp(pal + 2, font_palette[0] + 2, 2) == 0)
			return 0;
		if (memcmp(pal + 2, font_palette[1] + 2, 2) == 0)
			return 1;
		if (memcmp(pal + 2, font_palette[2] + 2, 2) == 0)
			return 2;
	}
	...
	else
	{
		...
		if (memcmp(pal + 2, font_palette[0] + 2, 5) == 0)
			return 6;
		if (memcmp(pal + 2, font_palette[3] + 2, 5) == 0)
			return 7;
	}
...
static void VideoA5_ExportFont(Font* font, const uint8* pal, int y, int* rety)
{
...
static void VideoA5_CreateFontCharacters(Font* font, const uint8* pal, int y, int* rety)
{
	const int WINDOW_W = 512;
	const int WINDOW_H = 1024;
...
static void VideoA5_InitFonts(unsigned char* buf)
{
	int y = 512;
...
static void VideoA5_InitWSA(unsigned char* buf)
{
	const int WINDOW_W = 512;
...
bool VideoA5_DrawWSA(void* wsa, int frame, int sx, int sy, int dx, int dy, int w, int h)
{
	...
	const unsigned char* buf = (const unsigned char*)GFX_Screen_Get_ByIndex(SCREEN_0);
...
static void VideoA5_InitCursor(unsigned char* buf)
{
...
void Video_InitMentatSprites(bool use_benepal)
{
	...
	const WindowID old_widget = (const WindowID)Widget_SetCurrentWidget(WINDOWID_RENDER_TEXTURE);
	const uint16 old_screen = GFX_Screen_SetActive(SCREEN_0);
	ALLEGRO_BITMAP* old_target = al_get_target_bitmap();

	unsigned char* buf = (unsigned char*)GFX_Screen_GetActive();
	...
	for (int i = 0; i < 15; i++)
	{
		const ShapeID shapeID = SHAPE_MENTAT_EYES + i;
	...
	GFX_Screen_SetActive((Screen)old_screen);
