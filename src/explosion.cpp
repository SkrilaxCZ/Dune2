...
static void Explosion_Func_TileDamage(Explosion* e, uint16 parameter)
{
	...
	if (type == LST_STRUCTURE || type == LST_DESTROYED_WALL)
		return;
	...
	if (!Sprite_IsUnveiled(overlaySpriteID))
		return;
...
{
	Audio_PlaySoundAtTile((SoundID)voiceID, e->position);
}
...
static void Explosion_StopAtPosition(uint16 packed)
{
	...
	for (int i = 1; i < s_explosions.num_elem; i++)
	{
		...
		if (e->commands == NULL)
			continue;
		if (Tile_PackTile(e->position) != packed)
			continue;
...
{
	if (explosionType > EXPLOSION_SPICE_BLOOM_TREMOR)
		return;

	uint16 packed = Tile_PackTile(position);
	Explosion_StopAtPosition(packed);

	Explosion* e = (Explosion*)BinHeap_Push(&s_explosions, Timer_GetTicks());
	if (e != NULL)
	{
		e->commands = g_table_explosion[explosionType];
		e->current = 0;
		e->spriteID = SHAPE_INVALID;
		e->position = position;
		e->isDirty = false;
...
void Explosion_Tick()
{
	...
	Explosion* e = (Explosion*)BinHeap_GetMin(&s_explosions);
	while ((e != NULL) && (e->timeOut <= curr_ticks))
	{
		while (e->commands != NULL)
		{
			...
			default:
			case EXPLOSION_STOP:
				Explosion_Func_Stop(e, parameter);
				break;

			case EXPLOSION_SET_SPRITE:
				Explosion_Func_SetSpriteID(e, parameter);
				break;
			case EXPLOSION_SET_TIMEOUT:
				Explosion_Func_SetTimeout(e, parameter);
				break;
			case EXPLOSION_SET_RANDOM_TIMEOUT:
				Explosion_Func_SetRandomTimeout(e, parameter);
				break;
			case EXPLOSION_MOVE_Y_POSITION:
				Explosion_Func_MoveYPosition(e, parameter);
				break;
			case EXPLOSION_TILE_DAMAGE:
				Explosion_Func_TileDamage(e, parameter);
				break;
			case EXPLOSION_PLAY_VOICE:
				Explosion_Func_PlayVoice(e, parameter);
				break;
			case EXPLOSION_SCREEN_SHAKE:
				Explosion_Func_ScreenShake(e, parameter);
				break;
			case EXPLOSION_SET_ANIMATION:
				Explosion_Func_SetAnimation(e, parameter);
				break;
			case EXPLOSION_BLOOM_EXPLOSION:
				Explosion_Func_BloomExplosion(e, parameter);
				break;
			}
		...
		e = (Explosion*)BinHeap_GetMin(&s_explosions);
	}
}
...
void Explosion_Draw()
{
	for (int i = 1; i < s_explosions.num_elem; i++)
	{
		...
		Shape_Draw(e->spriteID, x, y, (WindowID)2, 0xC000);
	}
}
