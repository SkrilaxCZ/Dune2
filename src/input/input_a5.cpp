...
static int InputA5_KeycodeToScancode(int kc)
{
...
bool InputA5_ProcessEvent(ALLEGRO_EVENT* event, bool apply_mouse_transform)
{
	int mouse_event = 0;
	...
	case ALLEGRO_EVENT_MOUSE_WARPED:
		 ...
		 */
		if (g_mousePanning)
			Mouse_EventHandler(false, g_mouseX, g_mouseY, 0, (Scancode)mouse_event);
		else
			Mouse_EventHandler(apply_mouse_transform, event->mouse.x, event->mouse.y, event->mouse.dz, (Scancode)mouse_event);
	...
	case ALLEGRO_EVENT_KEY_UP:
		{
			...
			Input_EventHandler((Scancode)sc);
		}
		break;
...
bool InputA5_Tick(bool apply_mouse_transform)
{
	bool redraw = false;
