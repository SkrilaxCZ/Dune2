...
void Mouse_EventHandler(bool apply_transform, int x, int y, int dz, Scancode state)
{
	...
	if (dz != 0)
		Input_EventHandler((Scancode)(SCANCODE_RELEASE | MOUSE_ZAXIS));
...
bool Mouse_InRegion(int x1, int y1, int x2, int y2)
{
	return (x1 <= g_mouseX && g_mouseX <= x2) &&
		(y1 <= g_mouseY && g_mouseY <= y2);
}

bool Mouse_InRegion_Div(ScreenDivID div, int x1, int y1, int x2, int y2)
{
	int mouseX, mouseY;
